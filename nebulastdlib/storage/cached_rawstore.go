...
import (
	...
	"github.com/nebulaclouds/nebula/nebulastdlib/ioutils"
...
func (s *cachedRawStore) Head(ctx context.Context, reference DataReference) (Metadata, error) {
	ctx, span := otelutils.NewSpan(ctx, otelutils.BlobstoreClientTracer, "nebulastdlib.storage.cachedRawStore/Head")
	defer span.End()
...
func (s *cachedRawStore) ReadRaw(ctx context.Context, reference DataReference) (io.ReadCloser, error) {
	ctx, span := otelutils.NewSpan(ctx, otelutils.BlobstoreClientTracer, "nebulastdlib.storage.cachedRawStore/ReadRaw")
	defer span.End()
...
func (s *cachedRawStore) WriteRaw(ctx context.Context, reference DataReference, size int64, opts Options, raw io.Reader) error {
	ctx, span := otelutils.NewSpan(ctx, otelutils.BlobstoreClientTracer, "nebulastdlib.storage.cachedRawStore/WriteRaw")
	defer span.End()
