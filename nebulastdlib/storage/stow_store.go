...
import (
	...
	"github.com/nebulaclouds/stow/azure"
	"github.com/nebulaclouds/stow/google"
	"github.com/nebulaclouds/stow/local"
	"github.com/nebulaclouds/stow/oracle"
	"github.com/nebulaclouds/stow/s3"
	"github.com/nebulaclouds/stow/swift"
)
...
func newStowRawStore(_ context.Context, cfg *Config, metrics *dataStoreMetrics) (RawStore, error) {
	...
	fn, ok := fQNFn[kind]
	if !ok {
		return nil, errs.Errorf("unsupported stow.kind [%s], add support in nebulastdlib?", kind)
	}
