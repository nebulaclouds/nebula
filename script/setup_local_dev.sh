...
: "${K3D_CLUSTER_NAME:="nebula"}" # name of k3d cluster to be used
...
: "${HELM_K8S_NAMESPACE:="nebula"}" # k8s namespace to use with helm
...
HELM_CHART="${DIR}/../charts/nebula-deps"

echo -e "\nUpdating helm dependencies"
helm dep update "${HELM_CHART}"

echo -e "\nApplying helm chart for nebula dependencies"
helm upgrade -n nebula --create-namespace $HELM_K8S_NAMESPACE "${HELM_CHART}" --kubeconfig "${K3D_KUBECONFIG_FILE_PATH}" --install --wait

echo -e "\nWaiting for helm chart rollouts to start"
timeout "$NEBULA_TIMEOUT" sh -c "until kubectl get namespace ${HELM_K8S_NAMESPACE} &> /dev/null; do sleep 1; done"  || ( echo >&2 "Timed out while waiting for the ${HELM_K8S_NAMESPACE} namespace to be created"; exit 1 )
timeout "$NEBULA_TIMEOUT" sh -c "until kubectl rollout status deployment minio -n ${HELM_K8S_NAMESPACE}  &> /dev/null; do sleep 1; done"  || ( echo >&2 "Timed out while waiting for the minio rollout to be created"; exit 1 )
timeout "$NEBULA_TIMEOUT" sh -c "until kubectl rollout status deployment postgres -n ${HELM_K8S_NAMESPACE}  &> /dev/null; do sleep 1; done"  || ( echo >&2 "Timed out while waiting for the postgres rollout to be created"; exit 1 )

echo -e "\nWaiting for Nebula dependencies deployment to start"
kubectl wait --for=condition=available deployment/minio deployment/postgres -n $HELM_K8S_NAMESPACE --timeout=5m &> /dev/null || ( echo >&2 "Timed out while waiting for the Nebula deployment to start"; exit 1 )

# Create directory to store certificate
mkdir -p /tmp/k8s-webhook-server/serving-certs

echo -e "\n*** Successfully set up local development environment."
echo -e "*** Run \"nebula start --config nebula_local_k3d.yaml\" to start your local nebula installation.\n"
