...
#######################
Authenticating in Nebula
#######################

.. tags:: Authentication, Infrastructure, Advanced

The Nebula platform consists of multiple components. Securing communication between each component is crucial to ensure
the integrity of the overall system.

The following diagram summarizes the components and their interactions as part of Nebula's auth implementation:


.. image:: https://raw.githubusercontent.com/nebulaclouds/static-resources/main/nebula/deployment/auth/nebula-auth-arch-v2.png


In summary, there are two main resources required for a complete auth flow in Nebula:

**An identity layer**

Using an implementation of the `Open ID Connect (OIDC) specification <https://openid.net/specs/openid-connect-core-1_0.html>`__, it enables clients to verify the identity of the end user based on the authentication performed by an Authorization Server. For this flow to work, you must first register Nebula as a new client (app) to the Identity Provider (IdP).

**An authorization server**

As defined by IETF's `RFC #6749 <https://datatracker.ietf.org/doc/html/rfc6749>`__, the authorization server's role is to issue *access tokens to the client after successfully authenticating the resource owner and obtaining authorization*. In this context, the *resource owner* is the end user of Nebula; and the *client* is the tool or component that intends to interact with ``nebulaadmin`` : ``nebulapropeller``, ``nebulaconsole`` or any of the CLI tools.

There are two supported options to use an authorization server in Nebula:
  
 * **Internal authorization server**: It comes pre-installed with Nebula and it is a suitable choice for quick start and testing purposes.
...
The following is required for non-sandbox (non ``nebulactl demo``) deployments:


* A public domain name (e.g. example.foobar.com)
* Routing of traffic from that domain name to the Kubernetes Nebula Ingress IP address

.. note::

   Checkout this `community-maintained guide <https://github.com/davidmirror-ops/nebula-the-hard-way/blob/main/docs/06-intro-to-ingress.md>`__ for more information about setting up Nebula in production, including Ingress.


Identity Providers Support
==========================

Nebula supports OAuth2 and OpenId Connect to secure the various connections:

* :ref:`OpenID Connect <deployment-auth-openid-appendix>`: used to secure user's authentication to ``nebulaadmin`` through the UI.
* :ref:`OAuth2 <deployment-auth-oauth2-appendix>`: used to secure connections from clients (i.e. ``pynebula``, ``nebulactl`` and
  ``nebulapropeller``) to the ``nebulaadmin`` service.
...
.. tabs::
   ...
   .. group-tab:: Okta
       ...
       3. Add sign-in redirect URIs: 
          ...
          - ``http://localhost:30081/callback`` for sandbox or ``https://<your deployment url>/callback`` for other Nebula deployment types.
          
       4. *Optional* - Add logout redirect URIs:
       
          - ``http://localhost:30081/logout`` for sandbox, ``https://<your deployment url>/callback`` for other Nebula deployment types).
   ...
   .. group-tab:: Microsoft Azure AD
       ...
       7. In the **Redirect URI** section select:
          ...
          - **Web** platform
          - Add ``http://localhost:30081/callback`` for sandbox or ``https://<your deployment url>/callback`` for other Nebula deployment types
...
.. tabs::
   ...
   .. group-tab:: nebula-binary
      ...
      1. Generate a random password to be used internally by ``nebulapropeller``
      ...
      .. code-block:: yaml
         ...
         auth:
          ...
          oidc:
            ...
            # Replace with the client ID and secret created for Nebula in your IdP
            clientId: <client_ID>
          ...
          authorizedUris:
          - https://<your-nebula-deployment-URL>
      ...
      .. prompt:: bash $
         ...
         helm upgrade <release-name> nebulaclouds/nebula-binary -n <your-namespace> --values <your-values-file>.yaml

      Where:
      
      * ``<release-name>`` is the name of your Helm release, typically ``nebula-backend``. You can find it using ``helm ls -n <your-namespace>``
        

      6. Verify that your Nebula deployment now requires successful login to your IdP to access the UI (``https://<your domain>/console``)

      7. For ``nebulactl`` / ``pynebula``, make sure that your local config file (``$HOME/.nebula/config.yaml``) includes the following option:
   ...
   .. group-tab:: nebula-core
      ...
      .. prompt:: bash $
         ...
         kubectl edit secret -n <nebula-namespace> nebula-admin-secrets

      Where ``nebula-namespace`` is the Kubernetes namespace where you have installed Nebula.
          ...
          # Accepting all domains for Sandbox installation
             ...
             auth:
              ...
              # Use the bcrypt hash generated for your random password
               ...
               authorizedUris:
               # Use the public URL of nebulaadmin (a DNS record pointing to your Ingress resource)
                 - https://<your-nebula-deployment-URL>
                 - http://nebulaadmin:80
      ...
      .. code-block:: yaml
         ...
         secrets:
          ...
          # If enabled is false, it's up to the user to create `nebula-secret-auth`
             enabled: true
      ...
      9. Restart ``nebulaadmin`` for the changes to take effect:

      .. prompt:: bash $

         kubectl rollout restart deployment/nebulaadmin -n nebula

      10. Restart ``nebulapropeller`` to start using authenticated requests:

      .. prompt:: bash $

          kubectl rollout restart deployment/nebulapropeller -n nebula

      11. Restart ``nebulascheduler``` to start using authenticated requests:

      .. prompt:: bash $

         kubectl rollout restart deployment/nebulascheduler -n nebula

      12. For nebulactl/pynebula, make sure that your local config file (``$HOME/.nebula/config.yaml``) includes the following option:
...
.. note::
   ...
   **Congratulations!**
   ...
   It should now be possible to go to Nebula UI and be prompted for authentication. Nebulactl should automatically pickup the change and start prompting for authentication as well.
...
As mentioned previously, Nebula ships with an internal authorization server; hence setting up an external Authorization Server is optional and dependent on your organization's security requirements.

In this section, you will find instructions on how to setup an OAuth2 Authorization Server in the different IdPs supported by Nebula:

.. note::

   **Google IdP**

   Google IdP does not offer an OAuth2 Authorization Server that could be used to protect external services (For example Nebula). In this case, Google offers a separate Cloud Product called Google Cloud Identity.
   Configuration for Cloud Identity is not included in this guide. If unavailable, setup can stop here and NebulaAdmin BuiltIn OAuth2 Authorization Server can be used instead.
...
.. tabs::
   ...
   .. group-tab:: Okta
       ...
       3. Assign an informative name and set the audience to the public URL of NebulaAdmin (e.g. https://example.foobar.com).
   
       .. note::
   
          The audience must exactly match one of the URIs in the ``authorizedUris`` section above
   
       4. Note down the **Issuer URI**; this will be used for all the ``baseUrl`` settings in the Nebula config.
       ...
       14. Create an integration for ``nebulactl``; it should be created with the **OIDC - OpenID Connect** sign-on method, and the **Native Application** type.
       15. Add ``http://localhost:53593/callback`` to the sign-in redirect URIs. The other options can remain as default.
       16. Assign this integration to any Okta users or groups who should be able to use the ``nebulactl`` tool.
       17. Note down the **Client ID**; there will not be a secret.
       18. Create an integration for ``nebulapropeller``; it should be created with the **OIDC - OpenID Connect** sign-on method and **Web Application** type.
       ...
       You should have three integrations total - one for the web interface (``nebulaconsole``), one for ``nebulactl``, and one for ``nebulapropeller``.
   ...
   .. group-tab:: Keycloak
       ...
       4. Create two clients (for `nebulactl` and `nebulapropeller`) to enable these clients to communicate with the service.
       5. `nebulactl` should be created with `Access Type Public` and standard flow enabled.
       6. `nebulaPropeller` should be created as an `Access Type Confidential`, enabling the standard flow
   ...
   .. group-tab:: Azure AD
       ...
       3. Add **Web** for nebulaconsole and nebulapropeller, **Mobile and desktop applications** for nebulactl.
       4. Add URL ``https://<console-url>/callback`` as the callback for Web
       5. Add URL ``http://localhost:53593/callback`` as the callback for nebulactl
...
Follow the steps in this section to configure `nebulaadmin` to use an external auth server. This section assumes that you have already completed and applied the configuration for the OIDC Identity Layer.

.. tabs::

   .. group-tab:: nebula-binary
      
      1. Go to the values YAML file you used to install Nebula using a Helm chart
      ...
      .. code-block:: yaml
         ...
         auth:
           ...
           internal:
           # Use the clientID generated by your IdP for the nebulapropeller app registration
             clientId: <nebulapropeller-client-id>
           #Use the secret generated by your IdP for nebulapropeller
             clientSecret: '<nebulapropeller-client-secret-non-encoded>'
           # Use the bcrypt hash for the clientSecret
             clientSecretHash: <-nebulapropeller-secret-bcrypt-hash>
           authorizedUris:
         ...
         inline:
           auth:
             appAuth:
               ...
               thirdPartyConfig:
                 nebulaClient:
                   # Use the clientID generated by your IdP for the `nebulactl` app registration
                   clientId: <nebulactl-client-id>
                   redirectUri: http://localhost:53593/callback
                   scopes:
                   - offline
                   - all
      ...
      .. prompt:: bash $
         ...
         helm upgrade  <release-name> nebulaclouds/nebula-core -n <your-namespace> --values <your-updated-values-filel>.yaml
   ...
   .. group-tab:: nebula-core
      ...
      .. code-block:: yaml
         ...
         secrets:
           adminOauthClientCredentials:
             ...
             # Replace with the client_secret provided by your IdP for nebulapropeller.
             clientSecret: <client_secret>
             # Replace with the client_id provided by provided by your IdP for nebulapropeller.
             clientId: <client_id>
      ...
      .. prompt:: bash $
         ...
         helm upgrade  <release-name> nebulaclouds/nebula-core -n <your-namespace> --values <your-updated-values-file>.yaml

   .. group-tab:: nebula-core with Azure AD
...
.. note::
   ...
   **Congratulations**
   ...
   At this point, every interaction with Nebula components -be it in the UI or CLI- should require a successful login to your IdP, where your security policies are maintained and enforced.


Disable Helm secret management
------------------------------

Alternatively, you can instruct Helm not to create and manage the secret for ``nebulapropeller``. In that case, you'll have to create it following these steps:
...
If your organization does any automated registration, then you'll need to authenticate with the `client credentials <https://datatracker.ietf.org/doc/html/rfc6749#section-4.4>`_ flow. After retrieving an access token from the IDP, you can send it along to `nebulaadmin`` as usual.

.. tabs::

   .. group-tab:: nebulactl
   
      Nebulactl's `config.yaml <https://docs.nebula.org/projects/nebulactl/en/stable/#configure>`_ can be
      ...
      .. code-block:: yaml
          ...
          admin:
              # Update with the Nebula's ingress endpoint (e.g. nebulaIngressIP for sandbox or example.foobar.com)
              # You must keep the 3 forward-slashes after dns:
              endpoint: dns:///<Nebula ingress url>
   
              # Update auth type to `Pkce` or `ClientSecret`
              authType: Pkce
   
              # Set to the clientId (will be used for both Pkce and ClientSecret flows)
              # Leave empty to use the value discovered through nebulaAdmin's Auth discovery endpoint.
              clientId: <Id>
              ...
              # If required, set the scopes needed here. Otherwise, nebulactl will discover scopes required for OpenID
              # Connect through nebulaAdmin's Auth discovery endpoint.
              # scopes: [ "scope1", "scope2" ]
   
      To read further about the available config options, please
      `visit here <https://github.com/nebulaclouds/nebulaidl/blob/master/clients/go/admin/config.go#L37-L64>`_
   
   .. group-tab:: Nebulakit / pynebula
   
      Nebulakit configuration variables are automatically designed to look up values from relevant environment variables.

      .. important::

         However, to aid with continuous integration use-cases, Nebulakit configuration can also reference other environment
         variables.
   
         For instance, if your CI system is not capable of setting custom environment variables like
         ``NEBULA_CREDENTIALS_CLIENT_SECRET`` but does set the necessary settings under a different variable, you may use
         ``export NEBULA_CREDENTIALS_CLIENT_SECRET_FROM_ENV_VAR=OTHER_ENV_VARIABLE`` to redirect the lookup. A
         ``NEBULA_CREDENTIALS_CLIENT_SECRET_FROM_FILE`` redirect is available as well, where the value should be the full
      ...
      The following is a listing of the Nebulakit configuration values we set in CI, along with a brief explanation.
   
      .. code-block:: bash
   
          # When using OAuth2 service auth, this is the username and password.
          export NEBULA_CREDENTIALS_CLIENT_ID=<client_id>
          export NEBULA_CREDENTIALS_CLIENT_SECRET=<client_secret>
   
          # This tells the SDK to use basic authentication. If not set, Nebulakit will assume you want to use the
          # standard OAuth based three-legged flow.
          export NEBULA_CREDENTIALS_AUTH_MODE=basic
   
          # This value should be set to conform to this
          # `header config <https://github.com/nebulaclouds/nebulaadmin/blob/12d6aa0a419ccec81b4c8289fd172e70a2ded525/auth/config/config.go#L124-L128>`_
          # on the Admin side.
          export NEBULA_CREDENTIALS_AUTHORIZATION_METADATA_KEY=<header name>
          ...
          export NEBULA_CREDENTIALS_OAUTH_SCOPES=<idp defined scopes>
   
          # Set this to force Nebulakit to use authentication, even if not required by Admin. This is useful as you're
          # rolling out the requirement.
          export NEBULA_PLATFORM_AUTH=True
