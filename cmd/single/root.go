...
var RootCmd = &cobra.Command{
	Use:   "nebula",
	Short: "Nebula cluster",
	Long: `
Use the start subcommand which will start the Nebula cluster

    nebula start --config  nebula_config.yaml
`,
...
func init() {
	...
	RootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is ./nebula.yaml)")
...
func initConfig(flags *pflag.FlagSet) error {
	configAccessor = viper.NewAccessor(config.Options{
		SearchPaths: []string{cfgFile, ".", "/etc/nebula/config", "$GOPATH/src/github.com/nebulaclouds/nebula"},
		StrictMode:  false,
	})
