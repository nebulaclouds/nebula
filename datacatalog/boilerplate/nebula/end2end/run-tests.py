...
from nebulakit.remote.executions import NebulaWorkflowExecution

WAIT_TIME = 10
MAX_ATTEMPTS = 200

# This dictionary maps the names found in the nebulasnacks manifest to a list of workflow names and
# inputs. This is so we can progressively cover all priorities in the original nebulasnacks manifest,
# starting with "core".
NEBULASNACKS_WORKFLOW_GROUPS: Mapping[str, List[Tuple[str, dict]]] = {
    "lite": [
        ("basics.hello_world.hello_world_wf", {}),
    ],
    "core": [
        # ("development_lifecycle.decks.image_renderer_wf", {}),
        # The chain_workflows example in nebulasnacks expects to be running in a sandbox.
        ...
        # Workflows that use nested executions cannot be launched via nebularemote.
        # This issue is being tracked in https://github.com/nebulaclouds/nebula/issues/1482.
        ...
        # Enums are not supported in nebularemote
...
def executions_finished(executions_by_wfgroup: Dict[str, List[NebulaWorkflowExecution]]) -> bool:
...
def sync_executions(remote: NebulaRemote, executions_by_wfgroup: Dict[str, List[NebulaWorkflowExecution]]):
    try:
        for executions in executions_by_wfgroup.values():
...
def report_executions(executions_by_wfgroup: Dict[str, List[NebulaWorkflowExecution]]):
    for executions in executions_by_wfgroup.values():
        for execution in executions:
            print(execution)
...
def schedule_workflow_groups(
    ...
    for wf_group in workflow_groups:
        workflows = NEBULASNACKS_WORKFLOW_GROUPS.get(wf_group, [])
...
def valid(workflow_group):
    """
    Return True if a workflow group is contained in NEBULASNACKS_WORKFLOW_GROUPS,
    False otherwise.
    """
    return workflow_group in NEBULASNACKS_WORKFLOW_GROUPS.keys()


def run(
    nebulasnacks_release_tag: str,
    priorities: List[str],
    config_file_path,
    terminate_workflow_on_failure: bool,
) -> List[Dict[str, str]]:
    ...
    # For a given release tag and priority, this function filters the workflow groups from the nebulasnacks
    # manifest file. For example, for the release tag "v0.2.224" and the priority "P0" it returns [ "core" ].
    manifest_url = (
        "https://raw.githubusercontent.com/nebulaclouds/nebulasnacks/" f"{nebulasnacks_release_tag}/nebula_tests_manifest.json"
    )
    ...
    results_by_wfgroup = schedule_workflow_groups(
        nebulasnacks_release_tag, valid_workgroups, remote, terminate_workflow_on_failure
    )
...
@click.argument("nebulasnacks_release_tag")
@click.argument("priorities")
@click.argument("config_file")
def cli(
    nebulasnacks_release_tag,
    priorities,
    config_file,
    return_non_zero_on_failure,
    terminate_workflow_on_failure,
    ...
    results = run(nebulasnacks_release_tag, priorities, config_file, terminate_workflow_on_failure)
