...
//     single unit or multiple units at once. At Nebula's current state, a resource means a logical
//     separation (e.g., a cluster) of an external service that allows a limited number of outstanding
//     requests to be sent to.
//
//   - Token: Nebula uses a token to serve as the placeholder to represent a unit of resource. Nebula resource manager
//     manages resources by managing the tokens of the resources.
//
//     2. Description
//     ResourceManager provides a task-type-specific pooling system for Nebula Tasks. Plugin writers can optionally
//     request for resources in their tasks, in single quantity.
//
//     3. Usage
//     A Nebula plugin registers the resources and the desired quota of each resource with ResourceRegistrar at the
//     setup time of Nebula Propeller. At the end of the setup time, Nebula Propeller builds a ResourceManager based on
//     these registration requests.
//
//     During runtime, the ResourceManager does two simple things: allocating tokens and releasing tokens. When a Nebula
...
//
//     4. Example
//     Nebula has a built-on Qubole plugin that allows Nebula tasks to send out Hive commands to Qubole.
