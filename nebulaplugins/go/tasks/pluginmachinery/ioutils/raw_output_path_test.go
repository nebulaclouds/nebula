...
func TestNewShardedRawOutputPath(t *testing.T) {
	...
	t.Run("", func(t *testing.T) {
		ss := NewConstantShardSelector([]string{"x"})
		sd, err := NewShardedRawOutputPath(ctx, ss, "s3://nebula", "unique", storage.URLPathConstructor{})
		assert.NoError(t, err)
		assert.Equal(t, storage.DataReference("s3://nebula/x/unique"), sd.GetRawOutputPrefix())
	})
