...
func (cfg K8sPluginConfig) GetPFlagSet(prefix string) *pflag.FlagSet {
	...
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "co-pilot.name"), defaultK8sConfig.CoPilot.NamePrefix, "Nebula co-pilot sidecar container name prefix. (additional bits will be added after this)")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "co-pilot.image"), defaultK8sConfig.CoPilot.Image, "Nebula co-pilot Docker Image FQN")
	...
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "default-pod-template-name"), defaultK8sConfig.DefaultPodTemplateName, "Name of the PodTemplate to use as the base for all k8s pods created by NebulaPropeller.")
