// Package config contains configuration for the nebulak8s module - which is global configuration for all Nebula K8s interactions.
// This config is under the subsection `k8s` and registered under the Plugin config
// All K8s based plugins can optionally use the nebulak8s module and this configuration allows controlling the defaults
...
import (
	...
	config2 "github.com/nebulaclouds/nebula/nebulastdlib/config"
)
...
var (
	defaultK8sConfig = K8sPluginConfig{
		...
		CoPilot: NebulaCoPilotConfig{
			NamePrefix:           "nebula-copilot-",
			Image:                "cr.nebula.org/nebulaclouds/nebulacopilot:v0.0.15",
			DefaultInputDataPath: "/var/nebula/inputs",
			InputVolumeName:      "nebula-inputs",
			DefaultOutputPath:    "/var/nebula/outputs",
			OutputVolumeName:     "nebula-outputs",
			CPU:                  "500m",
			Memory:               "128Mi",
...
type K8sPluginConfig struct {
	...
	// -------------------------------------------------------------------------------------------------------------
	// Default Configurations to be applied to all Pods launched by Nebula. These are always applied to every Pod.
	...
	// Default Tolerations that will be added to every Pod that is created by Nebula. These can be used in heterogeneous clusters, where one wishes to keep all pods created by Nebula on a separate
	// set of nodes.
	DefaultTolerations []v1.Toleration `json:"default-tolerations"  pflag:"-,Tolerations to be applied for every node that is launched by Nebula. Useful in non dedicated nebula clusters"`
	// Default Node Selector Labels for pods. These NodeSelector labels are added to all pods, created by Nebula, unless they are marked as interruptible (default of interruptible are different).
	DefaultNodeSelector map[string]string `json:"default-node-selector" pflag:"-,Defines a set of node selector labels to add to the all pods launched by Nebula. Useful in non dedicated Nebula clusters"`
	// Default Affinity that is applied to every pod that Nebula launches
	DefaultAffinity *v1.Affinity `json:"default-affinity,omitempty" pflag:"-,Defines default Affinity to be added for every Pod launched by Nebula. Useful in non dedicated Nebula clusters"`
	...
	// Nebula CoPilot Configuration
	CoPilot NebulaCoPilotConfig `json:"co-pilot" pflag:",Co-Pilot Configuration"`
	...
	// DefaultPodSecurityContext provides a default pod security context that should be applied for every pod that is launched by NebulaPropeller. This may not be applicable to all plugins. For
	// downstream plugins - i.e. TensorflowOperators may not support setting this, but Spark does.
	DefaultPodSecurityContext *v1.PodSecurityContext `json:"default-pod-security-context" pflag:"-,Optionally specify any default pod security context that should be applied to every Pod launched by NebulaPropeller."`

	// DefaultSecurityContext provides a default container security context that should be applied for the primary container launched and created by NebulaPropeller. This may not be applicable to all plugins. For
	//	// downstream plugins - i.e. TensorflowOperators may not support setting this, but Spark does.
	DefaultSecurityContext *v1.SecurityContext `json:"default-security-context" pflag:"-,Optionally specify a default security context that should be applied to every container launched/created by NebulaPropeller. This will not be applied to plugins that do not support it or to user supplied containers in pod tasks."`

	// EnableHostNetworkingPod is a binary switch to enable `hostNetwork: true` for all pods launched by Nebula.
	...
	// DefaultPodDNSConfig provides a default pod DNS config that that should be applied for the primary container launched and created by NebulaPropeller. This may not be applicable to all plugins. For
	//	// downstream plugins - i.e. TensorflowOperators may not support setting this.
	DefaultPodDNSConfig *v1.PodDNSConfig `json:"default-pod-dns-config" pflag:"-,Optionally specify a default DNS config that should be applied to every Pod launched by NebulaPropeller."`

	// DefaultPodTemplateName that serves as the base PodTemplate for all k8s pods (including
	// individual containers) that are creating by NebulaPropeller.
	DefaultPodTemplateName string `json:"default-pod-template-name" pflag:",Name of the PodTemplate to use as the base for all k8s pods created by NebulaPropeller."`
...
// NebulaCoPilotConfig specifies configuration for the Nebula CoPilot system. NebulaCoPilot, allows running nebulakit-less containers
// in K8s, where the IO is managed by the NebulaCoPilot sidecar process.
type NebulaCoPilotConfig struct {
	// Co-pilot sidecar container name
	NamePrefix string `json:"name" pflag:",Nebula co-pilot sidecar container name prefix. (additional bits will be added after this)"`
	// Docker image FQN where co-pilot binary is installed
	Image string `json:"image" pflag:",Nebula co-pilot Docker Image FQN"`
