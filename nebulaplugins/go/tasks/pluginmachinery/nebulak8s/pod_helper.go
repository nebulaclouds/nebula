...
import (
	...
	pluginserrors "github.com/nebulaclouds/nebula/nebulaplugins/go/tasks/errors"
	pluginsCore "github.com/nebulaclouds/nebula/nebulaplugins/go/tasks/pluginmachinery/core"
	"github.com/nebulaclouds/nebula/nebulaplugins/go/tasks/pluginmachinery/core/template"
...
// definition does not include any configuration injected by Nebula.
...
// ApplyNebulaPodConfiguration updates the PodSpec and ObjectMeta with various Nebula configuration. This includes
...
func ApplyNebulaPodConfiguration(ctx context.Context, tCtx pluginsCore.TaskExecutionContext, podSpec *v1.PodSpec, objectMeta *metav1.ObjectMeta, primaryContainerName string) (*v1.PodSpec, *metav1.ObjectMeta, error) {
	taskTemplate, err := tCtx.TaskReader().Read(ctx)
	if err != nil {
		logger.Warnf(ctx, "failed to read task information when trying to construct Pod, err: %s", err.Error())
		return nil, nil, err
	...
	// add nebula resource customizations to containers
	...
	for index, container := range podSpec.Containers {
		...
		if err := AddNebulaCustomizationsToContainer(ctx, templateParameters, resourceMode, &podSpec.Containers[index]); err != nil {
			return nil, nil, err
		}
...
// involves parsing the raw PodSpec definition and applying all Nebula configuration options.
func ToK8sPodSpec(ctx context.Context, tCtx pluginsCore.TaskExecutionContext) (*v1.PodSpec, *metav1.ObjectMeta, string, error) {
	...
	// add nebula configuration
	podSpec, objectMeta, err = ApplyNebulaPodConfiguration(ctx, tCtx, podSpec, objectMeta, primaryContainerName)
	if err != nil {
		return nil, nil, "", err
	}
...
// DemystifyPending is one the core functions, that helps NebulaPropeller determine if a pending pod is indeed pending,
...
// recover from failures, but Nebula pods are completely automated and should either run or fail
					...
					if !containerStatus.Ready {
						if containerStatus.State.Waiting != nil {
							...
							case "CreateContainerError":
								...
								// current time based on NebulaPropeller's clock. This
...
// This is useful in case of sidecars or pod jobs, where Nebula will monitor successful exit of a single container.
...
func DemystifyFailure(status v1.PodStatus, info pluginsCore.TaskInfo) (pluginsCore.PhaseInfo, error) {
	...
	// for now. This is required for NebulaPropeller to correctly transition
	// interruptible nodes to non-interruptible ones after the
	// `interruptible-failure-threshold` is exceeded. See:
	// https://github.com/nebulaclouds/nebulapropeller/blob/a3c6e91f19c19601a957b29891437112868845de/pkg/controller/nodes/node_exec_context.go#L213
