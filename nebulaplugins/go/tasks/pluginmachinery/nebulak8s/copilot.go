...
import (
	...
	core2 "github.com/nebulaclouds/nebula/nebulaplugins/go/tasks/pluginmachinery/core"
...
const (
	nebulaSidecarContainerName = "sidecar"
	nebulaInitContainerName    = "downloader"
)

var pTraceCapability = v1.Capability("SYS_PTRACE")

func NebulaCoPilotContainer(name string, cfg config.NebulaCoPilotConfig, args []string, volumeMounts ...v1.VolumeMount) (v1.Container, error) {
...
func AddCoPilotToContainer(ctx context.Context, cfg config.NebulaCoPilotConfig, c *v1.Container, iFace *core.TypedInterface, pilot *core.DataLoadingConfig) error {
...
func AddCoPilotToPod(ctx context.Context, cfg config.NebulaCoPilotConfig, coPilotPod *v1.PodSpec, iFace *core.TypedInterface, taskExecMetadata core2.TaskExecutionMetadata, inputPaths io.InputFilePaths, outputPaths io.OutputFilePaths, pilot *core.DataLoadingConfig) error {
	if pilot == nil || !pilot.Enabled {
		return nil
	}
	...
	if iFace != nil {
		if iFace.Inputs != nil && len(iFace.Inputs.Variables) > 0 {
			...
			downloader, err := NebulaCoPilotContainer(nebulaInitContainerName, cfg, args, inputsVolumeMount)
			if err != nil {
				return err
			}
		...
		if iFace.Outputs != nil && len(iFace.Outputs.Variables) > 0 {
			...
			sidecar, err := NebulaCoPilotContainer(nebulaSidecarContainerName, cfg, args, outputsVolumeMount)
			if err != nil {
				return err
			}
