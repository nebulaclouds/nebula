...
func TestNebulaCoPilotContainer(t *testing.T) {
	cfg := config.NebulaCoPilotConfig{
		NamePrefix:           "test-",
		Image:                "test",
	...
	t.Run("happy", func(t *testing.T) {
		c, err := NebulaCoPilotContainer("x", cfg, []string{"hello"})
	...
	t.Run("happy-vols", func(t *testing.T) {
		c, err := NebulaCoPilotContainer("x", cfg, []string{"hello"}, v1.VolumeMount{Name: "X", MountPath: "/"})
		assert.NoError(t, err)
	...
	t.Run("bad-res-cpu", func(t *testing.T) {
		old := cfg.CPU
		cfg.CPU = "x"
		_, err := NebulaCoPilotContainer("x", cfg, []string{"hello"}, v1.VolumeMount{Name: "X", MountPath: "/"})
		assert.Error(t, err)
		cfg.CPU = old
	})
...
func assertContainerHasVolumeMounts(t *testing.T, cfg config.NebulaCoPilotConfig, pilot *core.DataLoadingConfig, iFace *core.TypedInterface, c *v1.Container) {
	if iFace != nil {
...
func assertPodHasCoPilot(t *testing.T, cfg config.NebulaCoPilotConfig, pilot *core.DataLoadingConfig, iFace *core.TypedInterface, pod *v1.PodSpec) {
	for _, c := range pod.Containers {
		if c.Name == "test" {
			cntr := c
			assertContainerHasVolumeMounts(t, cfg, pilot, iFace, &cntr)
		} else {
			if c.Name == cfg.NamePrefix+nebulaInitContainerName || c.Name == cfg.NamePrefix+nebulaSidecarContainerName {
				if iFace != nil {
					vmap := map[string]v1.VolumeMount{}
					for _, v := range c.VolumeMounts {
						vmap[v.Name] = v
					...
					if iFace.Inputs != nil {
						...
						v, found := vmap[cfg.InputVolumeName]
						if c.Name == cfg.NamePrefix+nebulaInitContainerName {
							assert.Equal(t, path, v.MountPath, "Input Path does not match")
							assert.True(t, found, "Input volume mount expected but not found!")
						} else {
