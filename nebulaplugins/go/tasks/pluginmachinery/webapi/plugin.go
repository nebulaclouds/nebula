// Defines the  interfaces to implement to add a Web API Plugin (AsyncPlugin and SyncPlugin) to the Nebula system. A
// WebAPI plugin is a plugin that runs the compute for a task on a separate system through a web call (REST/Grpc...
// etc.). By implementing either the AsyncPlugin or SyncPlugin interfaces, the users of the Nebula system can then
...
type AsyncPlugin interface {
	...
	// Nebula will call this api at least once. It's important that the callee service is idempotent to ensure no
	// resource leakage or duplicate requests. Nebula has an in-memory cache that does a best effort idempotency
	// guarantee.
	...
	// Delete the object in the remote service using the resource key. Nebula will call this API at least once. If the
	// resource has already been deleted, the API should not fail.
	Delete(ctx context.Context, tCtx DeleteContext) error

	// Status checks the status of a given resource and translates it to a Nebula-understandable PhaseInfo. This API
