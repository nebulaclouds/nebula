...
import (
	...
	"github.com/nebulaclouds/nebula/nebulaplugins/go/tasks/plugins/array"
	config2 "github.com/nebulaclouds/nebula/nebulaplugins/go/tasks/plugins/array/awsbatch/config"
)

const (
	ArrayJobIndex       = "BATCH_JOB_ARRAY_INDEX_VAR_NAME"
	arrayJobIDFormatter = "%v:%v"
	failOnError         = "NEBULA_FAIL_ON_ERROR"
)
...
func NebulaTaskToBatchInput(ctx context.Context, tCtx pluginCore.TaskExecutionContext, jobDefinition string, cfg *config2.Config) (
	...
	// compile resources
	res, err := nebulak8s.ToK8sResourceRequirements(taskTemplate.GetContainer().GetResources())
	if err != nil {
		return nil, err
	}

	if overrideResources := tCtx.TaskExecutionMetadata().GetOverrides().GetResources(); overrideResources != nil {
		nebulak8s.MergeResources(*overrideResources, res)
	}
	...
	resources := nebulak8s.ApplyResourceOverrides(*res, *platformResources, assignResources)
...
func getEnvVarsForTask(ctx context.Context, execID pluginCore.TaskExecutionID, containerEnvVars []*core.KeyValuePair,
	...
	envVars := nebulak8s.DecorateEnvVars(ctx, nebulak8s.ToK8sEnvVar(containerEnvVars), nil, execID)
