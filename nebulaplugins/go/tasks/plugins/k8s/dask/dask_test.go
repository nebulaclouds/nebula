...
func TestBuildResourceDaskHappyPath(t *testing.T) {
	...
	// Nebula adds more environment variables to the runner
	...
	// Nebula adds more environment variables to the scheduler
	...
	// Nebula adds more environment variables to the worker
...
func TestBuildResourceDaskDefaultResoureRequirements(t *testing.T) {
	nebulaWorkflowResources := v1.ResourceRequirements{
		Requests: v1.ResourceList{
	...
	daskResourceHandler := daskResourceHandler{}
	taskTemplate := dummyDaskTaskTemplate("", nil, "")
	taskContext := dummyDaskTaskContext(taskTemplate, &nebulaWorkflowResources, nil, false)
	r, err := daskResourceHandler.BuildResource(context.TODO(), taskContext)
	assert.Nil(t, err)
	assert.NotNil(t, r)
	daskJob, ok := r.(*daskAPI.DaskJob)
	...
	// Job
	jobSpec := daskJob.Spec.Job.Spec
	assert.Equal(t, nebulaWorkflowResources, jobSpec.Containers[0].Resources)

	// Scheduler
	schedulerSpec := daskJob.Spec.Cluster.Spec.Scheduler.Spec
	assert.Equal(t, nebulaWorkflowResources, schedulerSpec.Containers[0].Resources)

	// Default Workers
	workerSpec := daskJob.Spec.Cluster.Spec.Worker.Spec
	assert.Equal(t, nebulaWorkflowResources, workerSpec.Containers[0].Resources)
...
func TestBuildResourcesDaskCustomResoureRequirements(t *testing.T) {
	...
	expectedResources, _ := nebulak8s.ToK8sResourceRequirements(&protobufResources)
...
func TestBuildResouceDaskUsePodTemplate(t *testing.T) {
	nebulak8s.DefaultPodTemplateStore.Store(podTemplate)
	daskResourceHandler := daskResourceHandler{}
	...
	// Cleanup
	nebulak8s.DefaultPodTemplateStore.Delete(podTemplate)
}
