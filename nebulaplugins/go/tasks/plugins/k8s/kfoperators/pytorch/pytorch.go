...
import (
	...
	"github.com/nebulaclouds/nebula/nebulaplugins/go/tasks/plugins/k8s/kfoperators/common"
)
...
func (pytorchOperatorResourceHandler) BuildResource(ctx context.Context, taskCtx pluginsCore.TaskExecutionContext) (client.Object, error) {
	...
	if err != nil {
		return nil, nebulaerr.Errorf(nebulaerr.BadTaskSpecification, "unable to fetch task specification [%v]", err.Error())
	} else if taskTemplate == nil {
		return nil, nebulaerr.Errorf(nebulaerr.BadTaskSpecification, "nil task specification")
	}
	...
	if taskTemplate.TaskTypeVersion == 0 {
		...
		if err != nil {
			return nil, nebulaerr.Errorf(nebulaerr.BadTaskSpecification, "invalid TaskSpecification [%v], Err: [%v]", taskTemplate.GetCustom(), err.Error())
		}

		replicaSpec, err := common.ToReplicaSpec(ctx, taskCtx, kubeflowv1.PytorchJobDefaultContainerName)
		if err != nil {
			return nil, nebulaerr.Errorf(nebulaerr.BadTaskSpecification, "Unable to create replica spec: [%v]", err.Error())
		}
		...
		if err != nil {
			return nil, nebulaerr.Errorf(nebulaerr.BadTaskSpecification, "Unable to create master replica spec: [%v]", err.Error())
		}
		...
		if err != nil {
			return nil, nebulaerr.Errorf(nebulaerr.BadTaskSpecification, "Unable to create worker replica spec: [%v]", err.Error())
		}
		...
		return nil, nebulaerr.Errorf(nebulaerr.BadTaskSpecification,
...
// always be aligned with the ElasticConfig defined in nebulaidl.
