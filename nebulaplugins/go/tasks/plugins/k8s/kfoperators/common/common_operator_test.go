...
func TestOverrideContainerSpecEmptyFields(t *testing.T) {
	...
	assert.Equal(t, []string{"pynebula-execute", "--task-module", "tests.nebulakit.unit.sdk.tasks.test_sidecar_tasks", "--task-name", "simple_sidecar_task", "--inputs", "{{.input}}", "--output-prefix", "{{.outputPrefix}}"}, podSpec.Containers[0].Args)
}
