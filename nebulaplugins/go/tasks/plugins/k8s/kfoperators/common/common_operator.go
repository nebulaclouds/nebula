...
import (
	...
	kfplugins "github.com/nebulaclouds/nebula/nebulaidl/gen/pb-go/nebulaidl/plugins/kubeflow"
	nebulaerr "github.com/nebulaclouds/nebula/nebulaplugins/go/tasks/errors"
...
func GetPhaseInfo(currentCondition commonOp.JobCondition, occurredAt time.Time,
	...
	case commonOp.JobFailed:
		...
		return pluginsCore.PhaseInfoRetryableFailure(nebulaerr.DownstreamSystemError, details, &taskPhaseInfo), nil
...
func OverridePrimaryContainerName(podSpec *v1.PodSpec, primaryContainerName string, defaultContainerName string) {
	...
	// https://github.com/nebulaclouds/nebulaplugins/blob/209c52d002b4e6a39be5d175bc1046b7e631c153/go/tasks/pluginmachinery/nebulak8s/container_helper.go#L116
...
func ParseRunPolicy(nebulaRunPolicy kfplugins.RunPolicy) commonOp.RunPolicy {
	runPolicy := commonOp.RunPolicy{}
	if nebulaRunPolicy.GetBackoffLimit() != 0 {
		var backoffLimit = nebulaRunPolicy.GetBackoffLimit()
		runPolicy.BackoffLimit = &backoffLimit
	}
	var cleanPodPolicy = ParseCleanPodPolicy(nebulaRunPolicy.GetCleanPodPolicy())
	runPolicy.CleanPodPolicy = &cleanPodPolicy
	if nebulaRunPolicy.GetActiveDeadlineSeconds() != 0 {
		var ddlSeconds = int64(nebulaRunPolicy.GetActiveDeadlineSeconds())
		runPolicy.ActiveDeadlineSeconds = &ddlSeconds
	}
	if nebulaRunPolicy.GetTtlSecondsAfterFinished() != 0 {
		var ttl = nebulaRunPolicy.GetTtlSecondsAfterFinished()
...
// Get k8s clean pod policy from nebula kubeflow plugins clean pod policy.
func ParseCleanPodPolicy(nebulaCleanPodPolicy kfplugins.CleanPodPolicy) commonOp.CleanPodPolicy {
	...
	return cleanPodPolicyMap[nebulaCleanPodPolicy]
}

// Get k8s restart policy from nebula kubeflow plugins restart policy.
func ParseRestartPolicy(nebulaRestartPolicy kfplugins.RestartPolicy) commonOp.RestartPolicy {
	...
	return restartPolicyMap[nebulaRestartPolicy]
}
...
func ToReplicaSpec(ctx context.Context, taskCtx pluginsCore.TaskExecutionContext, primaryContainerName string) (*commonOp.ReplicaSpec, error) {
	podSpec, objectMeta, oldPrimaryContainerName, err := nebulak8s.ToK8sPodSpec(ctx, taskCtx)
	if err != nil {
		return nil, nebulaerr.Errorf(nebulaerr.BadTaskSpecification, "Unable to create pod spec: [%v]", err.Error())
	}
...
func ToReplicaSpecWithOverrides(ctx context.Context, taskCtx pluginsCore.TaskExecutionContext, rs kfDistributedReplicaSpec, primaryContainerName string, isMaster bool) (*commonOp.ReplicaSpec, error) {
	taskCtxOptions := []nebulak8s.PluginTaskExecutionContextOption{}
	if rs != nil && rs.GetResources() != nil {
		resources, err := nebulak8s.ToK8sResourceRequirements(rs.GetResources())
		if err != nil {
			return nil, nebulaerr.Errorf(nebulaerr.BadTaskSpecification, "invalid TaskSpecification on Resources [%v], Err: [%v]", resources, err.Error())
		}
		taskCtxOptions = append(taskCtxOptions, nebulak8s.WithResources(resources))
	}
	newTaskCtx := nebulak8s.NewPluginTaskExecutionContext(taskCtx, taskCtxOptions...)
