...
var featureRegex = regexp.MustCompile(`^spark.((nebulaclouds)|(nebula)).(.+).enabled$`)
...
func getSparkConfig(taskCtx pluginsCore.TaskExecutionContext, sparkJob *plugins.SparkJob) map[string]string {
	...
	sparkConfig["spark.kubernetes.driverEnv.NEBULA_START_TIME"] = strconv.FormatInt(time.Now().UnixNano()/1000000, 10)

	return sparkConfig
}

func serviceAccountName(metadata pluginsCore.TaskExecutionMetadata) string {
	name := nebulak8s.GetServiceAccountNameFromTaskExecutionMetadata(metadata)
	if len(name) == 0 {
		name = sparkTaskType
	}
	return name
...
func createSparkPodSpec(taskCtx pluginsCore.TaskExecutionContext, podSpec *v1.PodSpec, container *v1.Container) *sparkOp.SparkPodSpec {
	...
	sparkEnv = append(sparkEnv, v1.EnvVar{Name: "NEBULA_MAX_ATTEMPTS", Value: strconv.Itoa(int(taskCtx.TaskExecutionMetadata().GetMaxAttempts()))})
...
func createDriverSpec(ctx context.Context, taskCtx pluginsCore.TaskExecutionContext, sparkConfig map[string]string) (*driverSpec, error) {
	...
	nonInterruptibleTaskCtx := nebulak8s.NewPluginTaskExecutionContext(taskCtx, nebulak8s.WithInterruptible(false))
	podSpec, _, primaryContainerName, err := nebulak8s.ToK8sPodSpec(ctx, nonInterruptibleTaskCtx)
	if err != nil {
		return nil, err
	}
	primaryContainer, err := nebulak8s.GetContainer(podSpec, primaryContainerName)
	if err != nil {
		return nil, err
	}
...
func createExecutorSpec(ctx context.Context, taskCtx pluginsCore.TaskExecutionContext, sparkConfig map[string]string) (*executorSpec, error) {
	podSpec, _, primaryContainerName, err := nebulak8s.ToK8sPodSpec(ctx, taskCtx)
	if err != nil {
		return nil, err
	}
	primaryContainer, err := nebulak8s.GetContainer(podSpec, primaryContainerName)
