...
const sparkUIAddress = "https://spark-ui.nebula"
...
var (
	dummySparkConf = map[string]string{
		...
		"spark.nebula.feature1.enabled":    "true",
		"spark.nebulaclouds.feature2.enabled": "true",
		"spark.nebulaclouds.feature3.enabled": "true",
...
func TestGetEventInfo(t *testing.T) {
	assert.NoError(t, setSparkConfig(&Config{
		LogConfig: LogConfig{
			User: logs.LogConfig{
				...
				CloudwatchTemplateURI: "https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#logStream:group=/kubernetes/nebula;prefix=var.log.containers.{{ .podName }};streamFilter=typeLogStreamPrefix",
				IsKubernetesEnabled:   true,
				KubernetesURL:         "k8s.com",
			},
			System: logs.LogConfig{
				IsCloudwatchEnabled:   true,
				CloudwatchTemplateURI: "https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#logStream:group=/kubernetes/nebula;prefix=system_log.var.log.containers.{{ .podName }};streamFilter=typeLogStreamPrefix",
			},
	...
	assert.Equal(t, "https://spark-ui.nebula", info.CustomInfo.Fields[sparkDriverUI].GetStringValue())
	...
	expectedLinks := []string{
		...
		"k8s.com/#!/log/spark-namespace/spark-pod/pod?namespace=spark-namespace",
		"https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#logStream:group=/kubernetes/nebula;prefix=var.log.containers.spark-pod;streamFilter=typeLogStreamPrefix",
		"https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#logStream:group=/kubernetes/nebula;prefix=system_log.var.log.containers.spark-app-name;streamFilter=typeLogStreamPrefix",
		"https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#logStream:group=/kubernetes/nebula;prefix=var.log.containers.spark-app-name;streamFilter=typeLogStreamPrefix",
		"https://spark-ui.nebula",
	}
	...
	assert.Equal(t, "https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#logStream:group=/kubernetes/nebula;prefix=var.log.containers.spark-app-name;streamFilter=typeLogStreamPrefix", info.Logs[0].Uri)

	assert.NoError(t, setSparkConfig(&Config{
		SparkHistoryServerURL: "spark-history.nebula",
		LogConfig: LogConfig{
			User: logs.LogConfig{
				IsCloudwatchEnabled:   true,
				CloudwatchTemplateURI: "https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#logStream:group=/kubernetes/nebula;prefix=var.log.containers.{{ .podName }};streamFilter=typeLogStreamPrefix",
	...
	assert.Equal(t, "spark-history.nebula/history/app-id", info.CustomInfo.Fields[sparkHistoryUI].GetStringValue())
	generatedLinks = make([]string, 0, len(info.Logs))
	for _, l := range info.Logs {
		generatedLinks = append(generatedLinks, l.Uri)
	}
	...
	expectedLinks = []string{
		...
		"https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#logStream:group=/kubernetes/nebula;prefix=system_log.var.log.containers.spark-app-name;streamFilter=typeLogStreamPrefix",
		"https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#logStream:group=/kubernetes/nebula;prefix=var.log.containers.spark-app-name;streamFilter=typeLogStreamPrefix",
		"spark-history.nebula/history/app-id",
	}
	...
	return &corev1.PodSpec{
		...
		Containers: []corev1.Container{
			...
			},
			{
				...
				Image: testImage,
				Args:  testArgs,
				Env:   nebulak8s.ToK8sEnvVar(dummyEnvVars),
			},
		},
	}
}
...
func dummySparkTaskTemplatePod(id string, sparkConf map[string]string, podSpec *corev1.PodSpec) *core.TaskTemplate {
	...
	return &core.TaskTemplate{
		...
		Config: map[string]string{
			nebulak8s.PrimaryContainerKey: "primary",
		},
		Custom: &structObj,
	}
}
...
func TestBuildResourceContainer(t *testing.T) {
	...
	assert.Equal(t, tolDriverDefault.Key, "x/nebula")
	...
	assert.Equal(t, tolExecInterrupt.Key, "x/nebula")
	...
	assert.Equal(t, tolExecDefault.Key, "x/nebula")
	...
	for confKey, confVal := range dummySparkConf {
		...
		if featureRegex.MatchString(confKey) && confKey != "spark.nebulaclouds.feature3.enabled" {
	...
	assert.Greater(t, len(sparkApp.Spec.SparkConf["spark.kubernetes.driverEnv.NEBULA_START_TIME"]), 1)
	assert.Equal(t, dummySparkConf["spark.nebulaclouds.feature3.enabled"], sparkApp.Spec.SparkConf["spark.nebulaclouds.feature3.enabled"])

	assert.Equal(t, len(findEnvVarByName(sparkApp.Spec.Driver.Env, "NEBULA_MAX_ATTEMPTS").Value), 1)
	...
	assert.Equal(t, sparkApp.Spec.Executor.Tolerations[0].Key, "x/nebula")
	assert.Equal(t, sparkApp.Spec.Executor.Tolerations[0].Value, "default")
	assert.Equal(t, sparkApp.Spec.Driver.Tolerations[0].Key, "x/nebula")
...
func TestBuildResourcePodTemplate(t *testing.T) {
	...
	// Application spec
	assert.Equal(t, nebulak8s.GetServiceAccountNameFromTaskExecutionMetadata(taskCtx.TaskExecutionMetadata()), *sparkApp.Spec.ServiceAccount)
	...
	assert.Equal(t, nebulak8s.GetServiceAccountNameFromTaskExecutionMetadata(taskCtx.TaskExecutionMetadata()), *sparkApp.Spec.Driver.ServiceAccount)
