...
func (rayJobResourceHandler) BuildResource(ctx context.Context, taskCtx pluginsCore.TaskExecutionContext) (client.Object, error) {
	...
	podSpec, objectMeta, primaryContainerName, err := nebulak8s.ToK8sPodSpec(ctx, taskCtx)
	if err != nil {
	...
	if primaryContainer == nil {
		return nil, nebulaerr.Errorf(nebulaerr.BadTaskSpecification, "Unable to get primary container from the pod: [%v]", err.Error())
	}
	...
	serviceAccountName := nebulak8s.GetServiceAccountNameFromTaskExecutionMetadata(taskCtx.TaskExecutionMetadata())
...
func (plugin rayJobResourceHandler) GetTaskPhase(ctx context.Context, pluginContext k8s.PluginContext, resource client.Object) (pluginsCore.PhaseInfo, error) {
	...
	case rayv1alpha1.JobDeploymentStatusFailedToGetOrCreateRayCluster:
		...
		return pluginsCore.PhaseInfoFailure(nebulaerr.TaskFailedWithError, reason, info), nil
