...
import (
	...
	errors2 "github.com/nebulaclouds/nebula/nebulaplugins/go/tasks/errors"
		...
		Containers: []v1.Container{
			{
				...
				Args: []string{"pynebula-execute", "--task-module", "tests.nebulakit.unit.sdk.tasks.test_sidecar_tasks", "--task-name", "simple_sidecar_task", "--inputs", "{{.input}}", "--output-prefix", "{{.outputPrefix}}"},
...
func TestBuildSidecarResource_TaskType2(t *testing.T) {
	...
	task := core.TaskTemplate{
		...
		Config: map[string]string{
			nebulak8s.PrimaryContainerKey: "primary container",
		},
...
func TestBuildSidecarResource(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.EqualValues(t, map[string]string{
		nebulak8s.PrimaryContainerKey: "a container",
		"a1":                         "a1",
	}, res.GetAnnotations())
	...
	testConfigs := []struct {
		...
		},
		{
			...
			core.TaskTemplate{
				...
				TaskTypeVersion: 1,
				Config: map[string]string{
					nebulak8s.PrimaryContainerKey: podSpec.Containers[0].Name,
				},
			},
		},
		{
...
func TestGetTaskSidecarStatus(t *testing.T) {
	...
	for podPhase, expectedTaskPhase := range testCases {
		...
		res.SetAnnotations(map[string]string{
			nebulak8s.PrimaryContainerKey: "PrimaryContainer",
		})
		taskCtx := getDummySidecarTaskContext(task, sidecarResourceRequirements, nil)
