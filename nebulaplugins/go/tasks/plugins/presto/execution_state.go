...
import (
	...
	pb "github.com/nebulaclouds/nebula/nebulaidl/gen/pb-go/nebulaidl/core"
...
type ExecutionState struct {
	...
	// the sync function is entirely opaque. Note that this field is completely orthogonal to Nebula system/node/task
...
func validatePrestoStatement(prestoJob plugins.PrestoQuery) error {
	if prestoJob.Statement == "" {
		return errors.Errorf(errors.BadTaskSpecification,
			"Query could not be found. Please ensure that you are at least on Nebulakit version 0.3.0 or later.")
	}
	return nil
}
...
func GetNextQuery(
	...
	case 0:
		...
		queryWrapTemplate := `
			CREATE TABLE hive.nebula_temporary_tables."%s_temp"
	...
	case 1:
		statement := fmt.Sprintf(`DROP TABLE hive.nebula_temporary_tables."%s"`, currentState.CurrentPrestoQuery.TempTableName)
