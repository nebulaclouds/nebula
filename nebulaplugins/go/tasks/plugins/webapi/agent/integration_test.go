...
func (m *MockClient) CreateTask(_ context.Context, createTaskRequest *admin.CreateTaskRequest, _ ...grpc.CallOption) (*admin.CreateTaskResponse, error) {
	expectedArgs := []string{"pynebula-fast-execute", "--output-prefix", "fake://bucket/prefix/nhv"}
...
func (m *MockClient) GetTask(_ context.Context, req *admin.GetTaskRequest, _ ...grpc.CallOption) (*admin.GetTaskResponse, error) {
	if req.GetTaskType() == "bigquery_query_job_task" {
		return &admin.GetTaskResponse{Resource: &admin.Resource{State: admin.State_SUCCEEDED, Outputs: &nebulaIdlCore.LiteralMap{
...
func TestEndToEnd(t *testing.T) {
	...
	template := nebulaIdlCore.TaskTemplate{
		...
		Custom: st,
		Target: &nebulaIdlCore.TaskTemplate_Container{
			Container: &nebulaIdlCore.Container{Args: []string{"pynebula-fast-execute", "--output-prefix", "{{.outputPrefix}}"}},
		},
	}
...
func getTaskContext(t *testing.T) *pluginCoreMocks.TaskExecutionContext {
	...
	tID.OnGetID().Return(nebulaIdlCore.TaskExecutionIdentifier{
		TaskId: &nebulaIdlCore.Identifier{
			ResourceType: nebulaIdlCore.ResourceType_TASK,
			Project:      "a",
			Domain:       "d",
			Name:         "n",
			Version:      "abc",
		},
		NodeExecutionId: &nebulaIdlCore.NodeExecutionIdentifier{
			NodeId: "node1",
			ExecutionId: &nebulaIdlCore.WorkflowExecutionIdentifier{
				Project: "a",
				Domain:  "d",
				Name:    "exec",
			},
