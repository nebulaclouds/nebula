...
import (
	...
	pluginErrors "github.com/nebulaclouds/nebula/nebulaplugins/go/tasks/errors"
	...
	"github.com/nebulaclouds/nebula/nebulaplugins/go/tasks/pluginmachinery/google"
	"github.com/nebulaclouds/nebula/nebulaplugins/go/tasks/pluginmachinery/ioutils"
	"github.com/nebulaclouds/nebula/nebulaplugins/go/tasks/pluginmachinery/webapi"
...
func (p Plugin) createImpl(ctx context.Context, taskCtx webapi.TaskExecutionContextReader) (*ResourceMetaWrapper,
	*ResourceWrapper, error) {
	...
	k8sServiceAccount := nebulak8s.GetServiceAccountNameFromTaskExecutionMetadata(taskCtx.TaskExecutionMetadata())
...
func createQueryJob(jobID string, custom *structpb.Struct, inputs *nebulaIdlCore.LiteralMap) (*bigquery.Job, error) {
...
func writeOutput(ctx context.Context, tCtx webapi.StatusContext, OutputLocation string) error {
	...
	return tCtx.OutputWriter().Put(ctx, ioutils.NewInMemoryOutputReader(
		&nebulaIdlCore.LiteralMap{
			Literals: map[string]*nebulaIdlCore.Literal{
				"results": {
					Value: &nebulaIdlCore.Literal_Scalar{
						Scalar: &nebulaIdlCore.Scalar{
							Value: &nebulaIdlCore.Scalar_StructuredDataset{
								StructuredDataset: &nebulaIdlCore.StructuredDataset{
									Uri: OutputLocation,
									Metadata: &nebulaIdlCore.StructuredDatasetMetadata{
...
func handleCreateError(createError *googleapi.Error, taskInfo *core.TaskInfo) core.PhaseInfo {
	...
	userExecutionError := &nebulaIdlCore.ExecutionError{
		Message: createError.Message,
		Kind:    nebulaIdlCore.ExecutionError_USER,
		Code:    code,
	}

	systemExecutionError := &nebulaIdlCore.ExecutionError{
		Message: createError.Message,
...
func (p Plugin) newBigQueryClient(ctx context.Context, identity google.Identity) (*bigquery.Service, error) {
	options := []option.ClientOption{
		...
		option.WithUserAgent(fmt.Sprintf("%s/%s", "nebulapropeller", "LATEST")),
	}
