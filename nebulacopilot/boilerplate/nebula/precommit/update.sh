...
# Try to find the `.git` directory, even if it's not in Nebula project root (as
...
if [ ! -e "${HOOKS_DIR}/prepare-commit-msg" ]; then
  ...
  ln -s "${HOOKS_DIR_RELPATH}/boilerplate/nebula/precommit/hooks/prepare-commit-msg" "${HOOKS_DIR}/prepare-commit-msg"
fi

if [ ! -e "${HOOKS_DIR}/pre-push" ]; then
  echo "Installing hook 'pre-push'"
  ln -s "${HOOKS_DIR_RELPATH}/boilerplate/nebula/precommit/hooks/pre-push" "${HOOKS_DIR}/pre-push"
fi
