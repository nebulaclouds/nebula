...
func (r *RootOptions) executeRootCmd() error {
	...
	version.LogBuildInformation("nebuladata")
...
func NewDataCommand() *cobra.Command {
	...
	command := &cobra.Command{
		Use:   "nebuladata",
		Short: "nebuladata is a simple go binary that can be used to retrieve and upload data from/to remote stow store to local disk.",
		Long:  `nebuladata when used with conjunction with nebulapropeller eliminates the need to have any nebula library installed inside the container`,
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			if err := rootOpts.initConfig(cmd, args); err != nil {
				return err
			}
			rootOpts.Scope = promutils.NewScope("nebula:data")
