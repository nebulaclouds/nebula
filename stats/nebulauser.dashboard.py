...
class NebulaUserDashboard(object):
        ...
        return Row(
            ...
            panels=[
                Graph(
                    ...
                    targets=[
                        Target(
                            expr='sum(rate(nebula:propeller:all:workflow:accepted{project=~"$project", domain=~"$domain", wf=~"$workflow"}[5m]))',
                            refId='A',
                        ),
                    ],
                    yAxes=YAxes(
                ...
                Graph(
                    ...
                    targets=[
                        Target(
                            expr='sum(rate(nebula:propeller:all:workflow:success_duration_ms_count{project=~"$project", domain=~"$domain", wf=~"$workflow"}[5m]))',
                            refId='A',
                        ),
                    ],
                    yAxes=YAxes(
                ...
                Graph(
                    ...
                    targets=[
                        Target(
                            expr='sum(rate(nebula:propeller:all:workflow:failure_duration_ms_count{project=~"$project", domain=~"$domain", wf=~"$workflow"}[5m]))',
                            refId='A',
                        ),
                    ],
                    yAxes=YAxes(
                ...
                Graph(
                    ...
                    targets=[
                        Target(
                            expr='sum(rate(nebula:propeller:all:workflow:workflow_aborted{project=~"$project", domain=~"$domain", wf=~"$workflow"}[5m]))',
                            refId='A',
                        ),
                    ],
                    yAxes=YAxes(
                ...
                Graph(
                    ...
                    targets=[
                        Target(
                            expr='sum(nebula:propeller:all:workflow:success_duration_ms{project=~"$project", domain=~"$domain", wf=~"$workflow"}) by (quantile)',
                            refId='A',
                        ),
                    ],
                ...
                Graph(
                    ...
                    targets=[
                        Target(
                            expr='sum(nebula:propeller:all:workflow:failure_duration_ms{project=~"$project", domain=~"$domain", wf=~"$workflow"}) by (quantile)',
                            refId='A',
                        ),
                    ],
                    yAxes=single_y_axis(format=MILLISECONDS_FORMAT),
                ...
                Graph(
                    ...
                    targets=[
                        Target(
                            expr='sum(nebula:propeller:all:node:queueing_latency_ms{project=~"$project", domain=~"$domain", wf=~"$workflow"}) by (quantile)',
                            refId='A',
                        ),
                    ],
                    yAxes=single_y_axis(format=MILLISECONDS_FORMAT),
        ...
        return Row(
            ...
            panels=[
                Graph(
                    ...
                    targets=[
                        Target(
                            expr='sum(rate(nebula:propeller:all:node:user_error_duration_ms_count{project=~"$project",domain=~"$domain",wf=~"$project:$domain:$workflow"}[5m]))',
                            refId='A',
                        ),
                    ],
                    yAxes=single_y_axis(format=SHORT_FORMAT),
                ...
                Graph(
                    ...
                    targets=[
                        Target(
                            expr='sum(rate(nebula:propeller:all:node:system_error_duration_ms_count{project=~"$project",domain=~"$domain",wf=~"$project:$domain:$workflow"}[5m]))',
                            refId='A',
                        ),
                    ],
                    yAxes=single_y_axis(format=SHORT_FORMAT),
    ...
    def create_all_rows(interval: int) -> typing.List[Row]:
        return [
            NebulaUserDashboard.workflow_stats(False),
            NebulaUserDashboard.quota_stats(True),
            NebulaUserDashboard.resource_stats(True),
            NebulaUserDashboard.errors(True),
        ]
...
project_template = Template(
    name="project",
    dataSource=DATASOURCE,
    query="label_values(nebula:propeller:all:collector:nebulaworkflow, project)",
    sort=True,
)
...
domain_template = Template(
    name="domain",
    dataSource=DATASOURCE,
    query="label_values(nebula:propeller:all:collector:nebulaworkflow, domain)",
    sort=True,
)
...
wf_template = Template(
    name="workflow",
    dataSource=DATASOURCE,
    query="label_values(nebula:propeller:all:collector:nebulaworkflow, wf)",
    sort=True,
    allValue=".*",
    includeAll=True,
)
...
dashboard = Dashboard(
    tags=["nebula", "prometheus", "nebulauser", "nebula-user"],
    inputs=[
        DataSourceInput(
    ...
    editable=False,
    title="Nebula User Dashboard (via Prometheus)",
    rows=NebulaUserDashboard.create_all_rows(interval=5),
    templating=Templating(list=[
        project_template,
        domain_template,
        wf_template,
    ]),
    description="Nebula User Dashboard. This is great to get a birds-eye and drill down view of executions in your Nebula cluster. Useful for the user.",
).auto_panel_ids()
