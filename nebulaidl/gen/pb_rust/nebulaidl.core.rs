    ...
    impl RuntimeType {
        ...
        pub fn as_str_name(&self) -> &'static str {
            match self {
                ...
                RuntimeType::NebulaSdk => "NEBULA_SDK",
            }
        }
        /// Creates an enum from field names used in the ProtoBuf definition.
        pub fn from_str_name(value: &str) -> ::core::option::Option<Self> {
            match value {
                ...
                "NEBULA_SDK" => Some(Self::NebulaSdk),
                _ => None,
            }
        }
    }
...
pub struct TaskMetadata {
    ...
    /// identically as, the default PodTemplate configured in NebulaPropeller.
    #[prost(string, tag="12")]
    ...
    // The logic of handling precedence will be done as part of nebulapropeller.
...
pub struct TaskTemplate {
    ...
    /// extensions are provided in the system, Nebula will resolve the this task to its TaskCategory and default the
...
pub struct Container {
    ...
    /// These will default to Nebula given paths. If provided, the system will not append known paths. If the task still
    /// needs nebula's inputs and outputs path, add $(NEBULA_INPUT_FILE), $(NEBULA_OUTPUT_FILE) wherever makes sense and the
    ...
    /// only from the local file-system and without having any reference to nebulaidl. This is supported only on K8s at the moment.
...
/// This configuration allows executing raw containers in Nebula using the Nebula CoPilot system.
/// Nebula CoPilot, eliminates the needs of nebulakit or sdk inside the container. Any inputs required by the users container are side-loaded in the input_path
...
pub struct DataLoadingConfig {
    ...
    /// Example, if the input interface needs (x: int, y: blob, z: multipart_blob) and the input path is '/var/nebula/inputs', then the file system will look like
    /// /var/nebula/inputs/inputs.<metadata format dependent -> .pb .json .yaml> -> Format as defined previously. The Blob and Multipart blob will reference local filesystem instead of remote locations
    /// /var/nebula/inputs/x -> X is a file that contains the value of x (integer) in string format
    /// /var/nebula/inputs/y -> Y is a file in Binary format
    /// /var/nebula/inputs/z/... -> Note Z itself is a directory
...
pub mod data_loading_config {
    ...
    pub enum LiteralMapFormat {
        ...
        Json = 0,
        Yaml = 1,
        /// Proto is a serialized binary of `core.LiteralMap` defined in nebulaidl/core
        Proto = 2,
    }
...
pub struct K8sPod {
    ...
    /// BETA: Optional configuration for DataLoading. If not specified, then default values are used.
    /// This makes it possible to to run a completely portable container, that uses inputs and outputs
    /// only from the local file-system and without having any reference to nebulakit. This is supported only on K8s at the moment.
    /// If data loading is enabled, then data will be mounted in accompanying directories specified in the DataLoadingConfig. If the directories
    /// are not specified, inputs will be mounted onto and outputs will be uploaded from a pre-determined file-system path. Refer to the documentation
    /// to understand the default paths.
    /// Only K8s
...
pub struct Sql {
    ...
    /// We use Nebula's Golang templating format for Query templating.
    /// Refer to the templating documentation.
    /// <https://docs.nebula.org/projects/cookbook/en/latest/auto/integrations/external_services/hive/hive.html#sphx-glr-auto-integrations-external-services-hive-hive-py>
    /// For example,
