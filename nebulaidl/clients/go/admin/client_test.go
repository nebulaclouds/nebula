...
import (
	...
	cachemocks "github.com/nebulaclouds/nebula/nebulaidl/clients/go/admin/cache/mocks"
	"github.com/nebulaclouds/nebula/nebulaidl/clients/go/admin/mocks"
	"github.com/nebulaclouds/nebula/nebulaidl/clients/go/admin/oauth"
	"github.com/nebulaclouds/nebula/nebulaidl/clients/go/admin/pkce"
	"github.com/nebulaclouds/nebula/nebulaidl/clients/go/admin/tokenorchestrator"
...
func TestGetAuthenticationDialOptionClientSecret(t *testing.T) {
	...
	t.Run("legal", func(t *testing.T) {
		...
		clientMetatadata := &service.PublicClientAuthConfigResponse{
			AuthorizationMetadataKey: "nebula_authorization",
		}
	...
	t.Run("error during nebula client", func(t *testing.T) {
		metadata := &service.OAuth2MetadataResponse{
			TokenEndpoint:   "/token",
			ScopesSupported: []string{"code", "all"},
		}
...
func ExampleClientSetBuilder() {
	ctx := context.Background()
	// Create a client set that initializes the connection with nebula admin and sets up Auth (if needed).
