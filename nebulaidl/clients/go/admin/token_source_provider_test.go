...
import (
	...
	tokenCacheMocks "github.com/nebulaclouds/nebula/nebulaidl/clients/go/admin/cache/mocks"
...
func TestNewTokenSourceProvider(t *testing.T) {
	...
	for _, test := range tests {
		...
		nebulaTokenSource, err := NewTokenSourceProvider(ctx, cfg, tokenCache, metadataClient)
		assert.True(t, metadataClient.AssertNumberOfCalls(t, "GetPublicClientConfig", test.expectedCallsPubEndpoint))
		assert.NoError(t, err)
		assert.NotNil(t, nebulaTokenSource)
		clientCredSourceProvider, ok := nebulaTokenSource.(ClientCredentialsTokenSourceProvider)
		assert.True(t, ok)
