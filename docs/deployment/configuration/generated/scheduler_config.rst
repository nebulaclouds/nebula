.. _nebulascheduler-config-specification:

#########################################
Nebula Scheduler Configuration
#########################################
...
- `nebulaadmin <#section-nebulaadmin>`_
...
OPTIONAL: Your IdP's token endpoint. It'll be discovered from nebula admin's OAuth Metadata endpoint if not provided.
...
Defines settings to instruct nebula cli tools (and optionally others) on what config to use to setup their client.
...
nebulaClient (`config.NebulaClientConfig`_)
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
public identifier for the app which handles authorization for a Nebula deployment
...
This is the callback uri registered with the app which handles authorization for a Nebula deployment
...
Whether the cluster resource sync is running in a standalone deployment and should call nebulaadmin service endpoints
...
.. code-block:: yaml
  ...
  co-pilot:
    cpu: 500m
    default-input-path: /var/nebula/inputs
    default-output-path: /var/nebula/outputs
    image: cr.nebula.org/nebulaclouds/nebulacopilot:v0.0.15
    input-vol-name: nebula-inputs
    memory: 128Mi
    name: nebula-copilot-
    output-vol-name: nebula-outputs
    start-timeout: 1m40s
    storage: ""
...
co-pilot (`config.NebulaCoPilotConfig`_)
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
Name of the PodTemplate to use as the base for all k8s pods created by NebulaPropeller.
...
config.NebulaCoPilotConfig
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

name (string)
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

Nebula co-pilot sidecar container name prefix. (additional bits will be added after this)
...
Nebula co-pilot Docker Image FQN

**Default Value**: 

.. code-block:: yaml

  cr.nebula.org/nebulaclouds/nebulacopilot:v0.0.15
...
MetadataPrefix should be used if all the metadata for Nebula executions should be stored under a specific prefix in CloudStorage. If not specified, the data will be stored in the base container directly.
...
a fully qualified storage path of the form s3://nebula/abc/..., where all data sandboxes should be stored.
...
Include the specified shard key label in the k8s NebulaWorkflow CRD label selector
...
Exclude the specified shard key label from the k8s NebulaWorkflow CRD label selector
...
Include the specified project label in the k8s NebulaWorkflow CRD label selector
...
Exclude the specified project label from the k8s NebulaWorkflow CRD label selector
...
Include the specified domain label in the k8s NebulaWorkflow CRD label selector
...
Exclude the specified domain label from the k8s NebulaWorkflow CRD label selector
...
Unique cluster id running this nebulapropeller instance with which to annotate execution events
...
create-nebulaworkflow-crd (bool)
------------------------------------------------------------------------------------------------------------------------

Enable creation of the NebulaWorkflow CRD on startup
...
local (`interfaces.NebulaSchedulerConfig`_)
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
interfaces.NebulaSchedulerConfig
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
local (`interfaces.NebulaWorkflowExecutorConfig`_)
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
interfaces.NebulaWorkflowExecutorConfig
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Kind of Stow backend to use. Refer to github/nebulaclouds/stow
...
Configuration for stow backend. Refer to github/nebulaclouds/stow
