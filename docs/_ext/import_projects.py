...
def update_sys_path_for_nebulakit(import_project_config: ImportProjectsConfig):
    # create nebulakit/_version.py file manually
    with open(f"{import_project_config.nebulakit_api_dir}/nebulakit/_version.py", "w") as f:
        f.write(f'__version__ = "dev"')


    # add nebulakit to python path
    nebulakit_dir = os.path.abspath(import_project_config.nebulakit_api_dir)
    nebulakit_src_dir = os.path.abspath(os.path.join(nebulakit_dir, "nebulakit"))
    plugins_dir = os.path.abspath(os.path.join(nebulakit_dir, "plugins"))

    sys.path.insert(0, nebulakit_src_dir)
    sys.path.insert(0, nebulakit_dir)
    ...
    for possible_plugin_dir in os.listdir(plugins_dir):
        ...
        plugin_path = os.path.abspath(os.path.join(dir_path, "nebulakitplugins"))
...
def import_projects(app: Sphinx, config: Config):
    ...
    for _dir in (
        ...
        import_projects_config.nebulakit_api_dir,
    ):
    ...
    # add nebulakit and plugins to path so that API reference can build
    update_sys_path_for_nebulakit(import_projects_config)
