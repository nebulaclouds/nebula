...
import (
	...
	impl2 "github.com/nebulaclouds/nebula/nebulaadmin/pkg/clusterresource/impl"
	...
	"github.com/nebulaclouds/nebula/nebulaadmin/pkg/executioncluster/impl"
	executionclusterIfaces "github.com/nebulaclouds/nebula/nebulaadmin/pkg/executioncluster/interfaces"
	...
	errors2 "github.com/nebulaclouds/nebula/nebulaadmin/pkg/repositories/errors"
	...
	admin2 "github.com/nebulaclouds/nebula/nebulaidl/clients/go/admin"
...
type controller struct {
	...
	adminDataProvider interfaces.NebulaAdminDataProvider
...
func populateTemplateValues(data map[string]runtimeInterfaces.DataSource) (templateValuesType, error) {
	...
	for templateVar, dataSource := range data {
		if templateVar == namespaceVariable || templateVar == projectVariable || templateVar == domainVariable {
			...
			collectedErrs = append(collectedErrs, errors.NewNebulaAdminErrorf(codes.InvalidArgument,
	...
	if len(collectedErrs) > 0 {
		return nil, errors.NewCollectedNebulaAdminError(codes.InvalidArgument, collectedErrs)
	}
...
func (c *controller) syncNamespace(ctx context.Context, project *admin.Project, domain *admin.Domain,
	...
	if err != nil {
		return ResourceSyncStats{}, errors.NewNebulaAdminErrorf(codes.Internal,
		...
		for _, target := range c.listTargets.GetValidTargets() {
			...
			if err != nil {
				if k8serrors.IsAlreadyExists(err) {
					...
					err := errors.NewNebulaAdminErrorf(codes.Internal,
	...
	if len(collectedErrs) > 0 {
		return stats, errors.NewCollectedNebulaAdminError(codes.Internal, collectedErrs)
	}
...
func (c *controller) createResourceFromTemplate(ctx context.Context, templateDir string,
	...
	if err != nil {
		logger.Warningf(ctx,
			...
			templateFileName, namespace, err)
		err := errors.NewNebulaAdminErrorf(
...
func (c *controller) Sync(ctx context.Context) error {
	...
	if len(errs) > 0 {
		return errors.NewCollectedNebulaAdminError(codes.Internal, errs)
	}
...
func NewClusterResourceController(adminDataProvider interfaces.NebulaAdminDataProvider, listTargets executionclusterIfaces.ListTargetsInterface, scope promutils.Scope) Controller {
...
func NewClusterResourceControllerFromConfig(ctx context.Context, scope promutils.Scope, configuration runtimeInterfaces.Configuration) (Controller, error) {
	...
	var adminDataProvider interfaces.NebulaAdminDataProvider
