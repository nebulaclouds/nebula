...
import (
	...
	scheduleInterfaces "github.com/nebulaclouds/nebula/nebulaadmin/pkg/async/schedule/interfaces"
...
func (m *LaunchPlanManager) CreateLaunchPlan(
	...
	if workflowModel.TypedInterface != nil && len(workflowModel.TypedInterface) > 0 {
		...
		if err != nil {
			...
			return nil, errors.NewNebulaAdminErrorf(codes.Internal, "failed to unmarshal workflow inputs")
		}
	}
...
func (m *LaunchPlanManager) updateLaunchPlanModelState(launchPlan *models.LaunchPlan, state admin.LaunchPlanState) error {
	...
	if err != nil {
		...
		return errors.NewNebulaAdminErrorf(codes.Internal, "Failed to unmarshal launch plan closure: %v", err)
	}
	...
	if err != nil {
		...
		return errors.NewNebulaAdminErrorf(codes.Internal, "Failed to marshal launch plan closure: %v", err)
	}
...
func (m *LaunchPlanManager) updateSchedules(
	...
	if err != nil {
		...
		return errors.NewNebulaAdminErrorf(codes.Internal, "failed to unmarshal newly enabled launch plan spec")
	}
	...
	if formerlyActiveLaunchPlan != nil {
		...
		if err != nil {
			return errors.NewNebulaAdminErrorf(codes.Internal, "failed to unmarshal formerly enabled launch plan spec")
		}
	}
...
func (m *LaunchPlanManager) UpdateLaunchPlan(ctx context.Context, request admin.LaunchPlanUpdateRequest) (
	*admin.LaunchPlanUpdateResponse, error) {
	...
	default:
		return nil, errors.NewNebulaAdminErrorf(
...
func (m *LaunchPlanManager) GetActiveLaunchPlan(ctx context.Context, request admin.ActiveLaunchPlanRequest) (
	*admin.LaunchPlan, error) {
	...
	if len(output.LaunchPlans) != 1 {
		return nil, errors.NewNebulaAdminErrorf(codes.NotFound, "No active launch plan could be found: %s:%s:%s", request.Id.Project, request.Id.Domain, request.Id.Name)
	}
...
func (m *LaunchPlanManager) ListLaunchPlanIds(ctx context.Context, request admin.NamedEntityIdentifierListRequest) (
	*admin.NamedEntityIdentifierList, error) {
	...
	offset, err := validation.ValidateToken(request.Token)
	if err != nil {
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "invalid pagination token %s", request.Token)
	}
	listLaunchPlansInput := repoInterfaces.ListResourceInput{
		Limit:         int(request.Limit),
		Offset:        offset,
