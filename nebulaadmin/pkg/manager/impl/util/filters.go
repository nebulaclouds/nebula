...
func ParseFilters(filterParams string, primaryEntity common.Entity) ([]common.InlineFilter, error) {
	...
	for _, filterExpression := range filterExpressions {
		...
		if !joinEntities.Has(referencedEntity) {
			return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "'%s' entity is not allowed in filters", referencedEntity)
		}

		if !entityColumns[referencedEntity].Has(field) {
			return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "'%s.%s' is invalid filter", referencedEntity, field)
		}
