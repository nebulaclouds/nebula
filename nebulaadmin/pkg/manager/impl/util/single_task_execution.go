...
const systemNamePrefix = ".nebulagen.%s"
...
func CreateOrGetWorkflowModel(
	...
	if err != nil {
		if ferr, ok := err.(errors.NebulaAdminError); !ok || ferr.Code() != codes.NotFound {
			return nil, err
		}
		...
		if err != nil {
			...
			// error.
			if ferr, ok := err.(errors.NebulaAdminError); !ok || ferr.Code() != codes.AlreadyExists {
				return nil, err
			}
		}
