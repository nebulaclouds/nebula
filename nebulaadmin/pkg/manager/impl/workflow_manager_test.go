...
import (
	...
	nebulaErrors "github.com/nebulaclouds/nebula/nebulaadmin/pkg/errors"
	...
	engine "github.com/nebulaclouds/nebula/nebulapropeller/pkg/compiler/common"
	mockScope "github.com/nebulaclouds/nebula/nebulastdlib/promutils"
	"github.com/nebulaclouds/nebula/nebulastdlib/storage"
)

const remoteClosureIdentifier = "s3://nebula/metadata/admin/remote closure id"
	...
	if !workflowOnGet {
		mockRepo.(*repositoryMocks.MockRepository).WorkflowRepo().(*repositoryMocks.MockWorkflowRepo).SetGetCallback(
			func(input interfaces.Identifier) (models.Workflow, error) {
				return models.Workflow{}, adminErrors.NewNebulaAdminError(codes.NotFound, "not found")
			})
	}
	return mockRepo
}
...
func TestCreateWorkflow_ExistingWorkflow_Different(t *testing.T) {
	...
	response, err := workflowManager.CreateWorkflow(context.Background(), request)
	assert.EqualError(t, err, "workflow with different structure already exists")
	nebulaErr := err.(nebulaErrors.NebulaAdminError)
	assert.Equal(t, codes.InvalidArgument, nebulaErr.Code())
	assert.Nil(t, response)
}
...
func TestGetWorkflow_TransformerError(t *testing.T) {
	...
	assert.Nil(t, workflow)
	assert.Equal(t, codes.Internal, err.(adminErrors.NebulaAdminError).Code())
}
...
func TestListWorkflows_MissingParameters(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Equal(t, codes.InvalidArgument, err.(adminErrors.NebulaAdminError).Code())
