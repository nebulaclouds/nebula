...
import (
	...
	cloudeventInterfaces "github.com/nebulaclouds/nebula/nebulaadmin/pkg/async/cloudevent/interfaces"
	eventWriter "github.com/nebulaclouds/nebula/nebulaadmin/pkg/async/events/interfaces"
	"github.com/nebulaclouds/nebula/nebulaadmin/pkg/async/notifications"
	notificationInterfaces "github.com/nebulaclouds/nebula/nebulaadmin/pkg/async/notifications/interfaces"
	"github.com/nebulaclouds/nebula/nebulaadmin/pkg/common"
	dataInterfaces "github.com/nebulaclouds/nebula/nebulaadmin/pkg/data/interfaces"
	"github.com/nebulaclouds/nebula/nebulaadmin/pkg/errors"
	"github.com/nebulaclouds/nebula/nebulaadmin/pkg/manager/impl/executions"
	...
	"github.com/nebulaclouds/nebula/nebulaadmin/pkg/manager/impl/util"
	"github.com/nebulaclouds/nebula/nebulaadmin/pkg/manager/impl/validation"
	...
	workflowengineInterfaces "github.com/nebulaclouds/nebula/nebulaadmin/pkg/workflowengine/interfaces"
...
func validateMapSize(maxEntries int, candidate map[string]string, candidateName string) error {
	...
	if len(candidate) > maxEntries {
		return errors.NewNebulaAdminErrorf(codes.InvalidArgument, "%s has too many entries [%v > %v]",
...
// TODO: Delete this code usage after the nebula v0.17.0 release
...
func (m *ExecutionManager) setCompiledTaskDefaults(ctx context.Context, task *core.CompiledTask,
	...
	cpu := nebulak8s.AdjustOrDefaultResource(taskResourceRequirements.Defaults.CPU, taskResourceRequirements.Limits.CPU,
	...
	memory := nebulak8s.AdjustOrDefaultResource(taskResourceRequirements.Defaults.Memory, taskResourceRequirements.Limits.Memory,
	...
	if !taskResourceRequirements.Defaults.EphemeralStorage.IsZero() ||
		...
		ephemeralStorage := nebulak8s.AdjustOrDefaultResource(taskResourceRequirements.Defaults.EphemeralStorage, taskResourceRequirements.Limits.EphemeralStorage,
	...
	if !taskResourceRequirements.Defaults.Storage.IsZero() ||
		...
		storageResource := nebulak8s.AdjustOrDefaultResource(taskResourceRequirements.Defaults.Storage, taskResourceRequirements.Limits.Storage,
	...
	if !taskResourceRequirements.Defaults.GPU.IsZero() ||
		...
		gpu := nebulak8s.AdjustOrDefaultResource(taskResourceRequirements.Defaults.GPU, taskResourceRequirements.Limits.GPU,
...
func (m *ExecutionManager) getExecutionConfig(ctx context.Context, request *admin.ExecutionCreateRequest,
	...
	// See NEBULA-2322 for more background information.
	...
	// In the case of reference_launch_plan subworkflow, the context comes from nebulapropeller instead of the user side, so user auth is missing.
	// We skip getUserIdentityFromContext but can still get ExecUserId because nebulapropeller passes it in the execution request.
	// https://github.com/nebulaclouds/nebulapropeller/blob/03a6672960ed04e7687ba4f790fee9a02a4057fb/pkg/controller/nodes/subworkflow/launchplan/admin.go#L114
...
func (m *ExecutionManager) CreateExecution(
	...
	*admin.ExecutionCreateResponse, error) {
	// Prior to  nebulaidl v0.15.0, Inputs was held in ExecutionSpec. Ensure older clients continue to work.
...
func (m *ExecutionManager) RelaunchExecution(
	...
	*admin.ExecutionCreateResponse, error) {
	...
	if len(existingExecutionModel.UserInputsURI) > 0 {
		...
		if err != nil {
			return nil, errors.NewNebulaAdminErrorf(codes.Internal, "failed to unmarshal spec")
		}
		inputs = spec.Inputs
	}
...
func (m *ExecutionManager) emitScheduledWorkflowMetrics(
	...
	if err != nil {
		// Timestamps are serialized by nebulaadmin and should always be valid
		return
	}
	...
	*admin.WorkflowExecutionEventResponse, error) {
	...
	if request.Event.Phase != core.WorkflowExecution_QUEUED {
		if wfExecPhase == request.Event.Phase {
			...
			return nil, errors.NewNebulaAdminErrorf(codes.AlreadyExists,
	...
	if common.IsExecutionTerminal(wfExecPhase) {
		...
		return nil, errors.NewNebulaAdminErrorf(codes.FailedPrecondition,
...
func (m *ExecutionManager) GetExecutionData(
	...
	// Prior to nebulaidl v0.15.0, Inputs were held in ExecutionClosure and were not offloaded. Ensure we can return the inputs as expected.
...
func (m *ExecutionManager) ListExecutions(
	...
	if err != nil {
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "invalid pagination token %s for ListExecutions",
			request.Token)
	}
...
// that are being manipulated have already been validated/manipulated by Nebula itself.
...
func (m *ExecutionManager) publishNotifications(ctx context.Context, request admin.WorkflowExecutionEventRequest,
	...
	if err != nil {
		...
		return errors.NewNebulaAdminErrorf(codes.Internal, "Failed to transform execution [%+v] with err: %v", request.Event.ExecutionId, err)
	}
	...
	for _, notification := range notificationsList {
		...
		if notification.GetEmail() != nil {
			...
			return errors.NewNebulaAdminErrorf(codes.Internal, "Unsupported notification type [%v] for execution [%+v]",
