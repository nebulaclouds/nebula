...
func ValidateMaxLengthStringField(field string, fieldName string, limit int) error {
	if len(field) > limit {
		return errors.NewNebulaAdminErrorf(codes.InvalidArgument, "%s cannot exceed %d characters", fieldName, limit)
	}
	return nil
}
...
func ValidateMaxMapLengthField(m map[string]string, fieldName string, limit int) error {
	if len(m) > limit {
		return errors.NewNebulaAdminErrorf(codes.InvalidArgument, "%s map cannot exceed %d entries", fieldName, limit)
	}
	return nil
}
...
func validateLabelsAlphanumeric(labels *admin.Labels) error {
	for key, value := range labels.Values {
		if errs := validation.IsQualifiedName(key); len(errs) > 0 {
			return errors.NewNebulaAdminErrorf(codes.InvalidArgument, "invalid label key [%s]: %v", key, errs)
		}
		if errs := validation.IsValidLabelValue(value); len(errs) > 0 {
			return errors.NewNebulaAdminErrorf(codes.InvalidArgument, "invalid label value [%s]: %v", value, errs)
		}
	}
	return nil
}
...
func ValidateVersion(version string) error {
	...
	if !strings.EqualFold(sanitizedVersion, version) {
		return errors.NewNebulaAdminErrorf(codes.InvalidArgument, "version [%s] must be url safe, cannot contains chars [%s]", version, uriReservedChars)
	}
	return nil
}
...
func validateLiteralMap(inputMap *core.LiteralMap, fieldName string) error {
	if inputMap != nil && len(inputMap.Literals) > 0 {
		for name, fixedInput := range inputMap.Literals {
			if name == "" {
				return errors.NewNebulaAdminErrorf(codes.InvalidArgument, "missing key in %s", fieldName)
			}
			if fixedInput == nil || fixedInput.GetValue() == nil {
				return errors.NewNebulaAdminErrorf(codes.InvalidArgument, "missing valid literal in %s %s", fieldName, name)
			}
...
func ValidateToken(token string) (int, error) {
	...
	if offset < 0 {
		return 0, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "Invalid token value: %s", token)
	}
	return offset, nil
}
...
func ValidateOutputData(outputData *core.LiteralMap, maxSizeInBytes int64) error {
	...
	return errors.NewNebulaAdminErrorf(codes.ResourceExhausted, "Output data size exceeds platform configured threshold (%+v > %v)", outputSizeInBytes, maxSizeInBytes)
}

func ValidateDatetime(literal *core.Literal) error {
	if literal == nil {
		return errors.NewNebulaAdminErrorf(codes.InvalidArgument, "Found invalid nil datetime")
	}
	...
	if err != nil {
		return errors.NewNebulaAdminErrorf(codes.InvalidArgument, err.Error())
	}
	return nil
}
