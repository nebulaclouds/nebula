...
var whitelistedTaskErr = errors.NewNebulaAdminErrorf(codes.InvalidArgument, "task type must be whitelisted before use")
			...
			if ok && platformLimitOk && limitQuantity.Value() > platformLimit.Value() {
				...
				return errors.NewNebulaAdminErrorf(codes.InvalidArgument,
					"Requested %v limit [%v] is greater than current limit set in the platform configuration"+
						" [%v]. Please contact Nebula Admins to change these limits or consult the configuration",
