...
func checkAndFetchExpectedInputForLaunchPlan(
	...
	for name, defaultInput := range defaultInputMap {
		...
		if !ok {
			return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "unexpected default_input %s", name)
	...
	for name, fixedInput := range fixedInputMap {
		value, ok := workflowExpectedInputMap[name]
		if !ok {
			return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "unexpected fixed_input %s", name)
		}
