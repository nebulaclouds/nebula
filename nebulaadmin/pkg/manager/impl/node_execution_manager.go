...
func (m *NodeExecutionManager) updateNodeExecutionWithEvent(
	...
	if nodeExecPhase == request.Event.Phase {
		...
		return updateFailed, errors.NewNebulaAdminErrorf(codes.AlreadyExists,
...
func (m *NodeExecutionManager) uploadDynamicWorkflowClosure(
	...
	if err != nil {
		return "", errors.NewNebulaAdminErrorf(codes.Internal,
...
func (m *NodeExecutionManager) CreateNodeEvent(ctx context.Context, request admin.NodeExecutionEventRequest) (
	*admin.NodeExecutionEventResponse, error) {
	...
	if err != nil {
		...
		if err != nil {
			if ferr, ok := err.(errors.NebulaAdminError); ok {
				return nil, errors.NewNebulaAdminErrorf(ferr.Code(),
	...
	if err != nil {
		if err.(errors.NebulaAdminError).Code() != codes.NotFound {
...
func (m *NodeExecutionManager) GetNodeExecutionData(
	...
	response := &admin.NodeExecutionGetDataResponse{
		...
		FullInputs:  inputs,
		FullOutputs: outputs,
		NebulaUrls:   common.NebulaURLsFromNodeExecutionID(*request.Id, nodeExecution.GetClosure() != nil && nodeExecution.GetClosure().GetDeckUri() != ""),
	}
	...
	if len(nodeExecutionModel.DynamicWorkflowRemoteClosureReference) > 0 {
		...
		if wf := closure.Primary; wf == nil {
			return nil, errors.NewNebulaAdminErrorf(codes.Internal, "Empty primary workflow definition in loaded dynamic workflow model.")
		} else if template := wf.Template; template == nil {
			return nil, errors.NewNebulaAdminErrorf(codes.Internal, "Empty primary workflow template in loaded dynamic workflow model.")
		} else {
