...
import (
	...
	eventWriterMocks "github.com/nebulaclouds/nebula/nebulaadmin/pkg/async/events/mocks"
	notificationMocks "github.com/nebulaclouds/nebula/nebulaadmin/pkg/async/notifications/mocks"
	...
	commonTestUtils "github.com/nebulaclouds/nebula/nebulaadmin/pkg/common/testutils"
	dataMocks "github.com/nebulaclouds/nebula/nebulaadmin/pkg/data/mocks"
	...
	runtimeIFaceMocks "github.com/nebulaclouds/nebula/nebulaadmin/pkg/runtime/interfaces/mocks"
	...
	workflowengineMocks "github.com/nebulaclouds/nebula/nebulaadmin/pkg/workflowengine/mocks"
...
func TestCreateExecutionPropellerFailure(t *testing.T) {
	...
	repository := getMockRepositoryForExecTest()
	setDefaultLpCallbackForExecTest(repository)
	expectedErr := nebulaAdminErrors.NewNebulaAdminErrorf(codes.Internal, "ABC")
	mockExecutor := workflowengineMocks.WorkflowExecutor{}
...
func TestCreateExecutionDatabaseFailure(t *testing.T) {
	...
	repository := getMockRepositoryForExecTest()
	setDefaultLpCallbackForExecTest(repository)
	expectedErr := nebulaAdminErrors.NewNebulaAdminErrorf(codes.Internal, "ABCD")
...
func TestRecoverExecution_RecoveredChildNode(t *testing.T) {
	...
	repository.ExecutionRepo().(*repositoryMocks.MockExecutionRepo).SetGetCallback(func(ctx context.Context, input interfaces.Identifier) (models.Execution, error) {
		...
		default:
			return models.Execution{}, nebulaAdminErrors.NewNebulaAdminErrorf(codes.InvalidArgument, "unexpected get for execution %s", input.Name)
		}
	})
...
func TestRecoverExecution_GetExistingInputsFailure(t *testing.T) {
	...
	// And verify response.
	assert.EqualError(t, err, "Unable to read WorkflowClosure from location s3://nebula/metadata/admin/remote closure id : foo")
}
...
func TestCreateWorkflowEvent_TerminalState(t *testing.T) {
	...
	assert.Nil(t, resp)
	assert.NotNil(t, err)
	adminError := err.(nebulaAdminErrors.NebulaAdminError)
...
func TestCreateWorkflowEvent_UpdateModelError(t *testing.T) {
	...
	assert.Nil(t, resp)
	assert.Equal(t, codes.Internal, err.(nebulaAdminErrors.NebulaAdminError).Code())
}
...
func TestListExecutions_MissingParameters(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Equal(t, codes.InvalidArgument, err.(nebulaAdminErrors.NebulaAdminError).Code())
...
func TestPluginOverrides_ResourceGetFailure(t *testing.T) {
	...
	db.ResourceRepo().(*repositoryMocks.MockResourceRepo).GetFunction = func(ctx context.Context, ID interfaces.ResourceID) (
		models.Resource, error) {
		return models.Resource{}, nebulaAdminErrors.NewNebulaAdminErrorf(codes.Aborted, "uh oh")
	}
	r := plugins.NewRegistry()
	r.RegisterDefault(plugins.PluginIDWorkflowExecutor, &defaultTestExecutor)
	execManager := NewExecutionManager(db, r, getMockExecutionsConfigProvider(), getMockStorageForExecTest(context.Background()), mockScope.NewTestScope(), mockScope.NewTestScope(), &mockPublisher, mockExecutionRemoteURL, nil, nil, nil, nil, &eventWriterMocks.WorkflowExecutionEventWriter{})
...
func TestCreateSingleTaskExecution(t *testing.T) {
					...
					Template: &core.TaskTemplate{
						...
						Metadata: &core.TaskMetadata{
							Runtime: &core.RuntimeMetadata{
								Type:    core.RuntimeMetadata_NEBULA_SDK,
								Version: "0.6.2",
								Flavor:  "python",
							},
						...
						Target: &core.TaskTemplate_Container{
							Container: &core.Container{
								...
								Env: []*core.KeyValuePair{
									{
										Key:   "NEBULA_INTERNAL_PROJECT",
										Value: "nebulakit",
									},
									{
										Key:   "NEBULA_INTERNAL_DOMAIN",
										Value: "production",
									},
									{
										Key:   "NEBULA_INTERNAL_NAME",
										Value: "simple_task",
									},
									{
										Key:   "NEBULA_INTERNAL_VERSION",
										Value: "12345",
									},
								},
							},
	...
	repository.LaunchPlanRepo().(*repositoryMocks.MockLaunchPlanRepo).SetGetCallback(func(input interfaces.Identifier) (models.LaunchPlan, error) {
		if launchplan == nil {
			return models.LaunchPlan{}, nebulaAdminErrors.NewNebulaAdminError(codes.NotFound, "launchplan not found")
		}
...
func TestGetExecutionConfigOverrides(t *testing.T) {
	...
	t.Run("application configuration", func(t *testing.T) {
		...
		t.Run("test pick up security context from admin system config", func(t *testing.T) {
			executionManager.config.ApplicationConfiguration().GetTopLevelConfig().K8SServiceAccount = "nebula-test"
			request := &admin.ExecutionCreateRequest{
				Project: workflowIdentifier.Project,
				Domain:  workflowIdentifier.Domain,
				Spec:    &admin.ExecutionSpec{},
			...
			execConfig, err := executionManager.getExecutionConfig(context.TODO(), request, nil)
			assert.NoError(t, err)
			assert.Equal(t, "nebula-test", execConfig.SecurityContext.RunAs.K8SServiceAccount)
