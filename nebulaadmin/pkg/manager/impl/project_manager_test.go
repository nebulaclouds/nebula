...
func TestProjectManager_CreateProject(t *testing.T) {
	...
	mockRepository.ProjectRepo().(*repositoryMocks.MockProjectRepo).CreateFunction = func(
		...
		assert.Equal(t, "nebula-project-id", namespace.Identifier)
		assert.Equal(t, "nebula-project-name", namespace.Name)
		assert.Equal(t, "nebula-project-description", namespace.Description)
		return nil
	}
	...
	_, err := projectManager.CreateProject(context.Background(), admin.ProjectRegisterRequest{
		Project: &admin.Project{
			Id:          "nebula-project-id",
			Name:        "nebula-project-name",
			Description: "nebula-project-description",
		},
	})
	assert.Nil(t, err)
