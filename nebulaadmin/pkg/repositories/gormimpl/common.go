...
func ValidateListInput(input interfaces.ListResourceInput) adminErrors.NebulaAdminError {
	if input.Limit == 0 {
...
func applyScopedFilters(tx *gorm.DB, inlineFilters []common.InlineFilter, mapFilters []common.MapFilter) (*gorm.DB, error) {
	for _, filter := range inlineFilters {
		...
		if !ok {
			return nil, adminErrors.NewNebulaAdminErrorf(codes.InvalidArgument,
