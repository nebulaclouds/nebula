...
func (r *TaskRepo) Get(ctx context.Context, input interfaces.Identifier) (models.Task, error) {
	...
	if errors.Is(tx.Error, gorm.ErrRecordNotFound) {
		return models.Task{}, nebulaAdminDbErrors.GetMissingEntityError(core.ResourceType_TASK.String(), &core.Identifier{
			Project: input.Project,
			Domain:  input.Domain,
			Name:    input.Name,
			Version: input.Version,
	...
	if tx.Error != nil {
		return models.Task{}, r.errorTransformer.ToNebulaAdminError(tx.Error)
	}
	return task, nil
}
...
func (r *TaskRepo) List(
	...
	timer.Stop()
	if tx.Error != nil {
		return interfaces.TaskCollectionOutput{}, r.errorTransformer.ToNebulaAdminError(tx.Error)
	}
...
func NewTaskRepo(
	db *gorm.DB, errorTransformer nebulaAdminDbErrors.ErrorTransformer, scope promutils.Scope) interfaces.TaskRepoInterface {
