...
func (s *SignalRepo) Get(ctx context.Context, input models.SignalKey) (models.Signal, error) {
	...
	if errors.Is(tx.Error, gorm.ErrRecordNotFound) {
		return models.Signal{}, adminerrors.NewNebulaAdminError(codes.NotFound, "signal does not exist")
	}
	if tx.Error != nil {
		return models.Signal{}, s.errorTransformer.ToNebulaAdminError(tx.Error)
	}
	return signal, nil
}
...
func (s *SignalRepo) GetOrCreate(ctx context.Context, input *models.Signal) error {
	...
	timer.Stop()
	if tx.Error != nil {
		return s.errorTransformer.ToNebulaAdminError(tx.Error)
	}
	return nil
}
...
func (s *SignalRepo) List(ctx context.Context, input interfaces.ListResourceInput) ([]models.Signal, error) {
	...
	timer.Stop()
	if tx.Error != nil {
		return nil, s.errorTransformer.ToNebulaAdminError(tx.Error)
	}
...
func (s *SignalRepo) Update(ctx context.Context, input models.SignalKey, value []byte) error {
	...
	if tx.RowsAffected == 0 {
		return adminerrors.NewNebulaAdminError(codes.NotFound, "signal does not exist")
	}
	return nil
}
...
func NewSignalRepo(
	db *gorm.DB, errorTransformer nebulaAdminDbErrors.ErrorTransformer, scope promutils.Scope) interfaces.SignalRepoInterface {
