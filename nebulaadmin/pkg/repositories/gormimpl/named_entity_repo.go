...
func (r *NamedEntityRepo) Get(ctx context.Context, input interfaces.GetNamedEntityInput) (models.NamedEntity, error) {
	...
	if !tableFound || !joinFound {
		return models.NamedEntity{}, adminErrors.NewNebulaAdminErrorf(codes.InvalidArgument, "Cannot get NamedEntityMetadata for resource type: %v", input.ResourceType)
	}
	...
	if tx.Error != nil {
		return models.NamedEntity{}, r.errorTransformer.ToNebulaAdminError(tx.Error)
	}
...
func (r *NamedEntityRepo) List(ctx context.Context, input interfaces.ListNamedEntityInput) (
	...
	tableName, tableFound := resourceTypeToTableName[input.ResourceType]
	if !tableFound {
		return interfaces.NamedEntityCollectionOutput{}, adminErrors.NewNebulaAdminErrorf(codes.InvalidArgument,
	...
	if tx.Error != nil {
		return interfaces.NamedEntityCollectionOutput{}, r.errorTransformer.ToNebulaAdminError(tx.Error)
	}
