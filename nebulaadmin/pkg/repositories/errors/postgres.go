...
func (p *postgresErrorTransformer) fromGormError(err error) nebulaAdminErrors.NebulaAdminError {
	switch err.Error() {
	case gorm.ErrRecordNotFound.Error():
		p.metrics.NotFound.Inc()
		return nebulaAdminErrors.NewNebulaAdminErrorf(codes.NotFound, "entry not found")
		// If we want to intercept other gorm errors, add additional case statements here.
	default:
		p.metrics.GormError.Inc()
		return nebulaAdminErrors.NewNebulaAdminErrorf(codes.Internal, unexpectedType, err)
	}
}

func (p *postgresErrorTransformer) ToNebulaAdminError(err error) nebulaAdminErrors.NebulaAdminError {
	...
	case uniqueConstraintViolationCode:
		...
		return nebulaAdminErrors.NewNebulaAdminErrorf(codes.AlreadyExists, uniqueConstraintViolation, pqError.Message)
	case undefinedTable:
		p.metrics.UndefinedTable.Inc()
		return nebulaAdminErrors.NewNebulaAdminErrorf(codes.InvalidArgument, unsupportedTableOperation, pqError.Message)
	default:
		p.metrics.PostgresError.Inc()
		return nebulaAdminErrors.NewNebulaAdminError(codes.Unknown, fmt.Sprintf(defaultPgError, pqError.Message))
	}
}
