...
import (
	...
	nebulaErrs "github.com/nebulaclouds/nebula/nebulaadmin/pkg/errors"
...
func CreateExecutionModel(input CreateExecutionModelInput) (*models.Execution, error) {
	...
	if err != nil {
		return nil, nebulaErrs.NewNebulaAdminErrorf(codes.Internal, "Failed to serialize execution spec: %v", err)
	}
	...
	if input.Error != nil {
		...
		var adminErr nebulaErrs.NebulaAdminError
	...
	if err != nil {
		return nil, nebulaErrs.NewNebulaAdminError(codes.Internal, "Failed to serialize launch plan status")
	}
...
func reassignCluster(ctx context.Context, cluster string, executionID *core.WorkflowExecutionIdentifier, execution *models.Execution) error {
	...
	if err != nil {
		return nebulaErrs.NewNebulaAdminErrorf(codes.Internal, "Failed to unmarshal execution spec: %v", err)
	}
	...
	if err != nil {
		return nebulaErrs.NewNebulaAdminErrorf(codes.Internal, "Failed to marshal execution spec: %v", err)
	}
...
func UpdateExecutionModelState(
	...
	if err != nil {
		return nebulaErrs.NewNebulaAdminErrorf(codes.Internal, "Failed to unmarshal execution closure: %v", err)
	}
	...
	if err != nil {
		return nebulaErrs.NewNebulaAdminErrorf(codes.Internal, "Failed to parse OccurredAt: %v", err)
	}
	...
	if !ignoreClusterFromEvent {
		if clusterReassignablePhases.Has(execution.Phase) {
			...
			return nebulaErrs.NewIncompatibleClusterError(ctx, errorMsg, execution.Cluster)
		}
	}
	...
	if err != nil {
		return nebulaErrs.NewNebulaAdminErrorf(codes.Internal, "Failed to marshal execution closure: %v", err)
	}
...
// by nebulapropeller. The metadata is preemptively saved at the time of the abort.
...
func FromExecutionModel(ctx context.Context, executionModel models.Execution, opts *ExecutionTransformerOptions) (*admin.Execution, error) {
	...
	if err = proto.Unmarshal(executionModel.Spec, &spec); err != nil {
		return nil, nebulaErrs.NewNebulaAdminErrorf(codes.Internal, "failed to unmarshal spec")
	}
	...
	if err = proto.Unmarshal(executionModel.Closure, &closure); err != nil {
		return nil, nebulaErrs.NewNebulaAdminErrorf(codes.Internal, "failed to unmarshal closure")
	}
