...
func CreateLaunchPlanModel(
	...
	if err != nil {
		return models.LaunchPlan{}, errors.NewNebulaAdminError(codes.Internal, "Failed to serialize launch plan spec")
	}
	closure, err := proto.Marshal(launchPlan.Closure)
	if err != nil {
		return models.LaunchPlan{}, errors.NewNebulaAdminError(codes.Internal, "Failed to serialize launch plan closure")
	}
...
func FromLaunchPlanModel(model models.LaunchPlan) (*admin.LaunchPlan, error) {
	...
	if err != nil {
		return nil, errors.NewNebulaAdminError(codes.Internal, "failed to unmarshal spec")
	}

	var closure admin.LaunchPlanClosure
	err = proto.Unmarshal(model.Closure, &closure)
	if err != nil {
		return nil, errors.NewNebulaAdminError(codes.Internal, "failed to unmarshal closure")
	}
	createdAt, err := ptypes.TimestampProto(model.CreatedAt)
	if err != nil {
		return nil, errors.NewNebulaAdminErrorf(codes.Internal, "failed to read created at")
	}
	updatedAt, err := ptypes.TimestampProto(model.UpdatedAt)
	if err != nil {
		return nil, errors.NewNebulaAdminErrorf(codes.Internal, "failed to read updated at")
	}
	if model.State != nil {
