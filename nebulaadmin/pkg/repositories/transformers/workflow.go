...
func CreateWorkflowModel(request admin.WorkflowCreateRequest, remoteClosureIdentifier string,
	...
	if request.Spec != nil && request.Spec.Template != nil && request.Spec.Template.Interface != nil {
		...
		if err != nil {
			return models.Workflow{}, errors.NewNebulaAdminError(codes.Internal, "Failed to serialize workflow spec")
		}
...
func FromWorkflowModel(workflowModel models.Workflow) (admin.Workflow, error) {
	...
	if err != nil {
		return admin.Workflow{}, errors.NewNebulaAdminErrorf(codes.Internal, "failed to read created at timestamp")
	}
	...
	if len(workflowModel.TypedInterface) > 0 {
		...
		if err != nil {
			return admin.Workflow{}, errors.NewNebulaAdminErrorf(codes.Internal, fmt.Sprintf("failed to unmarshal workflow %v interface. Error message: %v", workflowModel.ID, err.Error()))
		}
	}
