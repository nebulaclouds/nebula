...
func CreateTaskModel(
	...
	if err != nil {
		return models.Task{}, errors.NewNebulaAdminError(codes.Internal, "Failed to serialize task closure")
	}
	var taskType string
...
func FromTaskModel(taskModel models.Task) (admin.Task, error) {
	...
	if err != nil {
		return admin.Task{}, errors.NewNebulaAdminError(codes.Internal, "failed to unmarshal closure")
	}

	createdAt, err := ptypes.TimestampProto(taskModel.CreatedAt)
	if err != nil {
		return admin.Task{}, errors.NewNebulaAdminErrorf(codes.Internal, "failed to serialize created at")
	}
