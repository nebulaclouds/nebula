...
func (cfg ServerConfig) GetPFlagSet(prefix string) *pflag.FlagSet {
	...
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "thirdPartyConfig.nebulaClient.clientId"), defaultServerConfig.DeprecatedThirdPartyConfig.NebulaClientConfig.ClientID, "public identifier for the app which handles authorization for a Nebula deployment")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "thirdPartyConfig.nebulaClient.redirectUri"), defaultServerConfig.DeprecatedThirdPartyConfig.NebulaClientConfig.RedirectURI, "This is the callback uri registered with the app which handles authorization for a Nebula deployment")
	cmdFlags.StringSlice(fmt.Sprintf("%v%v", prefix, "thirdPartyConfig.nebulaClient.scopes"), defaultServerConfig.DeprecatedThirdPartyConfig.NebulaClientConfig.Scopes, "Recommended scopes for the client to request.")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "thirdPartyConfig.nebulaClient.audience"), defaultServerConfig.DeprecatedThirdPartyConfig.NebulaClientConfig.Audience, "Audience to use when initiating OAuth2 authorization requests.")
