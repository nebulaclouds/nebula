...
func RemoteClusterConfig(host string, auth runtimeInterfaces.Auth) (*restclient.Config, error) {
	...
	if err != nil {
		return nil, errors.NewNebulaAdminErrorf(codes.Internal, "Failed to get auth token: %+v", err)
	}

	caCert, err := auth.GetCA()
	if err != nil {
		return nil, errors.NewNebulaAdminErrorf(codes.Internal, "Failed to get auth CA: %+v", err)
	}
...
func GetRestClientConfig(kubeConfigPathString, master string,
	...
	if kubeConfigPathString != "" {
		...
		if err != nil {
			return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "Error building kubeconfig: %v", err)
		}
		...
		if err != nil {
			return nil, errors.NewNebulaAdminErrorf(codes.Internal, "Cannot get incluster kubeconfig : %v", err.Error())
		}
