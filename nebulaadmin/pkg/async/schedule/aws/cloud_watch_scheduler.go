...
import (
	...
	"github.com/nebulaclouds/nebula/nebulaadmin/pkg/async/schedule/aws/interfaces"
	...
	appInterfaces "github.com/nebulaclouds/nebula/nebulaadmin/pkg/runtime/interfaces"
...
func getScheduleExpression(schedule admin.Schedule) (string, error) {
	...
	return "", errors.NewNebulaAdminErrorf(codes.InvalidArgument, "unrecognized schedule expression")
}
...
func (s *cloudWatchScheduler) AddSchedule(ctx context.Context, input scheduleInterfaces.AddScheduleInput) error {
	if input.Payload == nil {
		...
		return errors.NewNebulaAdminError(codes.InvalidArgument, "payload serialization function cannot be nil")
	}
	...
	if err != nil {
		...
		return errors.NewNebulaAdminErrorf(codes.Internal, "failed to add rule to cloudwatch with err: %v", err)
	}
	...
	if err != nil {
		...
		return errors.NewNebulaAdminErrorf(codes.Internal, "failed to add target for event schedule with err: %v", err)
...
func (s *cloudWatchScheduler) RemoveSchedule(ctx context.Context, input scheduleInterfaces.RemoveScheduleInput) error {
	...
	if err != nil {
		if isResourceNotFoundException(err) {
			...
			return errors.NewNebulaAdminErrorf(codes.Internal, "failed to remove cloudwatch target %s with err: %v", name, err)
		}
	}
	if output != nil && output.FailedEntryCount != nil && *output.FailedEntryCount > 0 {
		...
		return errors.NewNebulaAdminErrorf(codes.Internal, "failed to remove cloudwatch target %s with %v errs",
			name, *output.FailedEntryCount)
	}
