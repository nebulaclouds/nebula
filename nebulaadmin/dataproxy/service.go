...
func (s Service) CreateUploadLocation(ctx context.Context, req *service.CreateUploadLocationRequest) (
	*service.CreateUploadLocationResponse, error) {
	...
	if len(req.Project) == 0 || len(req.Domain) == 0 {
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "project and domain are required parameters")
	}

	// At least one of the hash or manually given prefix must be provided.
	if len(req.FilenameRoot) == 0 && len(req.ContentMd5) == 0 {
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument,
	...
	if len(req.Filename) > 0 && len(req.FilenameRoot) > 0 {
		...
		if err != nil {
			return nil, errors.NewNebulaAdminErrorf(codes.Internal, "failed to create storage location, Error: %v", err)
		}
		metadata, err := s.dataStore.Head(ctx, knownLocation)
		if err != nil {
			return nil, errors.NewNebulaAdminErrorf(codes.Internal, "failed to check if file exists at location [%s], Error: %v", knownLocation.String(), err)
		}
		if metadata.Exists() {
			...
			if len(req.ContentMd5) == 0 {
				return nil, errors.NewNebulaAdminErrorf(codes.AlreadyExists, "file already exists at location [%v], specify a matching hash if you wish to rewrite", knownLocation)
			}
	...
	if expiresIn := req.ExpiresIn; expiresIn != nil {
		if !expiresIn.IsValid() {
			return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "expiresIn [%v] is invalid", expiresIn)
		}

		if expiresIn.AsDuration() > s.cfg.Upload.MaxExpiresIn.Duration {
			return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "expiresIn [%v] cannot exceed max allowed expiration [%v]",
	...
	if err != nil {
		return nil, errors.NewNebulaAdminErrorf(codes.Internal, "failed to create shardedStorageLocation, Error: %v", err)
	}
	...
	if err != nil {
		return nil, errors.NewNebulaAdminErrorf(codes.Internal, "failed to create a signed url. Error: %v", err)
	}
...
func (s Service) CreateDownloadLink(ctx context.Context, req *service.CreateDownloadLinkRequest) (
	...
	if req, err = s.validateCreateDownloadLinkRequest(req); err != nil {
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "error while validating request. Error: %v", err)
	}
	...
	if nodeExecutionIDEnvelope, casted := req.GetSource().(*service.CreateDownloadLinkRequest_NodeExecutionId); casted {
		...
		if err != nil {
			return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "failed to find node execution [%v]. Error: %v", nodeExecutionIDEnvelope.NodeExecutionId, err)
		}
		...
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "unsupported source [%v]", reflect.TypeOf(req.GetSource()))
	}

	if len(nativeURL) == 0 {
		return nil, errors.NewNebulaAdminErrorf(codes.Internal, "no deckUrl found for request [%+v]", req)
	}
...
func (s Service) CreateDownloadLocation(ctx context.Context, req *service.CreateDownloadLocationRequest) (
	*service.CreateDownloadLocationResponse, error) {
	...
	if err := s.validateCreateDownloadLocationRequest(req); err != nil {
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "error while validating request: %v", err)
	}
...
func (s Service) validateResolveArtifactRequest(req *service.GetDataRequest) error {
	if len(req.GetNebulaUrl()) == 0 {
		return fmt.Errorf("source is required. Provided empty string")
	}
	if !strings.HasPrefix(req.GetNebulaUrl(), "nebula://") {
...
// The one coming from the node execution doesn't have this as this is not data encapsulated in the nebula url.
func (s Service) GetCompleteTaskExecutionID(ctx context.Context, taskExecID core.TaskExecutionIdentifier) (*core.TaskExecutionIdentifier, error) {
	...
	if err != nil {
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "failed to list task executions [%v]. Error: %v", taskExecID, err)
	}
	if len(taskExecs.TaskExecutions) == 0 {
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "no task executions were listed [%v]. Error: %v", taskExecID, err)
	}
...
func (s Service) GetTaskExecutionID(ctx context.Context, attempt int, nodeExecID core.NodeExecutionIdentifier) (*core.TaskExecutionIdentifier, error) {
	...
	if err != nil {
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "failed to list task executions [%v]. Error: %v", nodeExecID, err)
	}
	if len(taskExecs.TaskExecutions) == 0 {
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "no task executions were listed [%v]. Error: %v", nodeExecID, err)
	}
	taskExec := taskExecs.TaskExecutions[0]
	return taskExec.Id, nil
}
...
func (s Service) GetDataFromNodeExecution(ctx context.Context, nodeExecID core.NodeExecutionIdentifier, ioType common.ArtifactType, name string) (
	*service.GetDataResponse, error) {
	...
	if name != "" {
		...
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "name [%v] not found in node execution [%v]", name, nodeExecID)
	}
	return &service.GetDataResponse{
...
func (s Service) GetDataFromTaskExecution(ctx context.Context, taskExecID core.TaskExecutionIdentifier, ioType common.ArtifactType, name string) (
	*service.GetDataResponse, error) {
	...
	if ioType == common.ArtifactTypeI {
		...
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "deck type cannot be specified with a retry attempt, just use the node instead")
	}
	...
	if name != "" {
		...
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "name [%v] not found in task execution [%v]", name, taskExecID)
	}
...
func (s Service) GetData(ctx context.Context, req *service.GetDataRequest) (
	*service.GetDataResponse, error) {
	...
	logger.Debugf(ctx, "resolving nebula url query: %s", req.GetNebulaUrl())
	err := s.validateResolveArtifactRequest(req)
	if err != nil {
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "failed to validate resolve artifact request. Error: %v", err)
	}

	execution, err := common.ParseNebulaURLToExecution(req.GetNebulaUrl())
	if err != nil {
		return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "failed to parse artifact url Error: %v", err)
	}
	...
	return nil, errors.NewNebulaAdminErrorf(codes.InvalidArgument, "failed to parse get data request %v", req)
}
