...
var RootCmd = &cobra.Command{
	Use:   "nebulascheduler",
	Short: "Nebula native scheduler to run cron and fixed rate scheduled workflows",
	Long: `
Use the run subcommand which will start the scheduler by connecting to DB containing schedules

    nebulascheduler run --config  nebulaadmin_config.yaml  --admin.endpoint dns:///localhost:8089 --admin.insecure
`,
...
func init() {
	// allows `$ nebulascheduler --logtostderr` to work
