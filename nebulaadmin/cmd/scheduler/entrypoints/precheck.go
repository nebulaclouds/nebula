...
const (
	healthCheckSuccess = "Health check passed, Nebulaadmin is up and running"
...
var preCheckRunCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err != nil {
			logger.Errorf(ctx, "Nebula native scheduler precheck failed due to %v\n", err)
			return err
		}

		healthCheckResponse, err := clientSet.HealthServiceClient().Check(ctx,
			&grpc_health_v1.HealthCheckRequest{Service: "nebulaadmin"})
		if err != nil {
			return err
		}
