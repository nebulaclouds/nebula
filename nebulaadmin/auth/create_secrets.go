...
`,
		Example: `
Create a secret using default name (nebula-admin-auth) in default namespace
nebulaadmin secret create --fromPath=/path/in/container

Override an existing secret if one exists (reads secrets from default path /etc/secrets/):
nebulaadmin secret create --name "my-auth-secrets" --force
`,
		RunE: func(cmd *cobra.Command, args []string) error {
			return persistSecrets(context.Background(), cmd.Flags())
		},
	}
	cmd.Flags().StringVar(&secretName, "name", "nebula-admin-auth", "Chooses secret name to create/update")
		...
		if forceUpdate {
			...
			if needsUpdate {
				...
				if err != nil && kubeErrors.IsConflict(err) {
					logger.Infof(ctx, "Another instance of nebulaadmin has updated the same secret. Ignoring this update")
					err = nil
				}
			}
		}
