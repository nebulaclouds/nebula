...
func TestHashNebulaClientSecret(t *testing.T) {
...
func TestDefaultConfig(t *testing.T) {
	...
	assert.Equal(t, DefaultConfig.AppAuth.SelfAuthServer.StaticClients["nebula-cli"].ID, "nebula-cli")
}

func TestCompare(t *testing.T) {
	hasher := &fosite.BCrypt{WorkFactor: 6}
	err := hasher.Compare(context.Background(), DefaultConfig.AppAuth.SelfAuthServer.StaticClients["nebulapropeller"].Secret, []byte("foobar"))
	assert.Error(t, err)
}
