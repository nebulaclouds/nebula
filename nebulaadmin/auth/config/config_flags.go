...
func (cfg Config) GetPFlagSet(prefix string) *pflag.FlagSet {
	...
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "appAuth.thirdPartyConfig.nebulaClient.clientId"), DefaultConfig.AppAuth.ThirdParty.NebulaClientConfig.ClientID, "public identifier for the app which handles authorization for a Nebula deployment")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "appAuth.thirdPartyConfig.nebulaClient.redirectUri"), DefaultConfig.AppAuth.ThirdParty.NebulaClientConfig.RedirectURI, "This is the callback uri registered with the app which handles authorization for a Nebula deployment")
	cmdFlags.StringSlice(fmt.Sprintf("%v%v", prefix, "appAuth.thirdPartyConfig.nebulaClient.scopes"), DefaultConfig.AppAuth.ThirdParty.NebulaClientConfig.Scopes, "Recommended scopes for the client to request.")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "appAuth.thirdPartyConfig.nebulaClient.audience"), DefaultConfig.AppAuth.ThirdParty.NebulaClientConfig.Audience, "Audience to use when initiating OAuth2 authorization requests.")
	return cmdFlags
}
