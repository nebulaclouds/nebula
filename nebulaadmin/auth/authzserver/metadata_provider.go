...
import (
	...
	authConfig "github.com/nebulaclouds/nebula/nebulaadmin/auth/config"
...
func (s OAuth2MetadataProvider) GetPublicClientConfig(context.Context, *service.PublicClientAuthConfigRequest) (*service.PublicClientAuthConfigResponse, error) {
	return &service.PublicClientAuthConfigResponse{
		ClientId:                 s.cfg.AppAuth.ThirdParty.NebulaClientConfig.ClientID,
		RedirectUri:              s.cfg.AppAuth.ThirdParty.NebulaClientConfig.RedirectURI,
		Scopes:                   s.cfg.AppAuth.ThirdParty.NebulaClientConfig.Scopes,
		AuthorizationMetadataKey: s.cfg.GrpcAuthorizationHeader,
		Audience:                 s.cfg.AppAuth.ThirdParty.NebulaClientConfig.Audience,
	}, nil
}
...
func sendAndRetryHTTPRequest(ctx context.Context, client *http.Client, url string, retryAttempts int, retryDelay time.Duration) (*http.Response, error) {
	...
	retryableOauthMetadataError := nebulaErrors.NewNebulaAdminError(codes.Internal, "Failed to get oauth metadata.")
