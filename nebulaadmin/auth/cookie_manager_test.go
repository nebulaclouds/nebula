...
import (
	...
	"github.com/nebulaclouds/nebula/nebulaadmin/auth/config"
...
func TestCookieManager(t *testing.T) {
	...
	t.Run("set_token_cookies", func(t *testing.T) {
		...
		c := w.Result().Cookies()
		assert.Equal(t, "nebula_at", c[0].Name)
		assert.Equal(t, "nebula_idt", c[1].Name)
		assert.Equal(t, "nebula_rt", c[2].Name)
	})
	...
	t.Run("retrieve_token_values_wrong_key", func(t *testing.T) {
		...
		assert.EqualError(t, err, "[EMPTY_OAUTH_TOKEN] Error reading existing secure cookie [nebula_idt]. Error: [SECURE_COOKIE_ERROR] Error reading secure cookie nebula_idt, caused by: securecookie: error - caused by: crypto/aes: invalid key size 75")
	})
	...
	t.Run("set_user_info", func(t *testing.T) {
		...
		assert.Equal(t, "nebula_user_info", cookies[0].Name)
	})
	...
	t.Run("set_auth_code", func(t *testing.T) {
		...
		cookies := w.Result().Cookies()
		require.Len(t, cookies, 1)
		assert.Equal(t, "nebula_auth_code", cookies[0].Name)
	})
}
