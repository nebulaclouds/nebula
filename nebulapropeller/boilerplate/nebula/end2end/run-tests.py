...
def executions_finished(
    executions_by_wfgroup: Dict[str, List[NebulaWorkflowExecution]]
) -> bool:
...
def sync_executions(
    remote: NebulaRemote, executions_by_wfgroup: Dict[str, List[NebulaWorkflowExecution]]
):
    try:
        for executions in executions_by_wfgroup.values():
...
def run(
    ...
    manifest_url = (
        "https://raw.githubusercontent.com/nebulaclouds/nebulasnacks/"
        f"{nebulasnacks_release_tag}/nebula_tests_manifest.json"
    )
