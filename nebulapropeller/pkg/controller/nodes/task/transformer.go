...
// This is used by nebulaadmin to indicate that map tasks now report subtask metadata individually.
...
func ToTaskExecutionEvent(input ToTaskExecutionEventInputs) (*event.TaskExecutionEvent, error) {
	...
	if !nebulak8sConfig.GetK8sPluginConfig().SendObjectEvents {
		// For back compat with old versions of nebulaadmin, populate the deprecated reason field.
		// Setting SendObjectEvents to true assumes that nebulaadmin is using the new reasons field.
