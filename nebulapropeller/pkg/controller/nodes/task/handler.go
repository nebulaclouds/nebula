...
import (
	...
	pluginMachinery "github.com/nebulaclouds/nebula/nebulaplugins/go/tasks/pluginmachinery"
...
func (t *Handler) Setup(ctx context.Context, sCtx interfaces.SetupContext) error {
	...
	// Not every task type will have a default plugin specified in the nebulapropeller config.
	...
	for _, p := range enabledPlugins {
		...
		// For every default plugin for a task type specified in nebulapropeller config we validate that the plugin's
...
func (t Handler) ResolvePlugin(ctx context.Context, ttype string, executionConfig v1alpha1.ExecutionConfig) (pluginCore.Plugin, error) {
	...
	// registered in this deployment of nebulapropeller.
...
func (t Handler) Handle(ctx context.Context, nCtx interfaces.NodeExecutionContext) (handler.Transition, error) {
	ctx, span := otelutils.NewSpan(ctx, otelutils.NebulaPropellerTracer, "pkg.controller.nodes.task.Handler/HandleTask")
	defer span.End()
