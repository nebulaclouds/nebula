...
// Contains the set of enabled plugins for this nebulapropeller deployment along with default plugin handlers
	...
	for pluginName, taskTypes := range pluginDefaultForTaskType {
		if !enabledPluginsNames.Has(pluginName) {
			logger.Errorf(context.TODO(), "Cannot set default plugin [%s] for task types [%+v] when it is not "+
				"configured to be an enabled plugin. Please double check the nebulapropeller config.", pluginName, taskTypes)
