...
import (
	...
	"github.com/nebulaclouds/nebula/nebulaplugins/go/tasks/pluginmachinery/catalog/mocks"
...
func TestHandler_newTaskExecutionContext(t *testing.T) {
	...
	assert.Equal(t, storage.DataReference("s3://sandbox/x/fpmmhh6q/_nebulacheckpoints"), anotherTaskExecCtx.ow.GetCheckpointPrefix())
	assert.Equal(t, storage.DataReference("s3://sandbox/x/fpqmhlei/_nebulacheckpoints"), anotherTaskExecCtx.ow.GetPreviousCheckpointsPrefix())
...
func TestComputePreviousCheckpointPath(t *testing.T) {
	...
	t.Run("attempt-1-nCtx", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Equal(t, storage.DataReference("s3://sandbox/x/name-n1-0/_nebulacheckpoints"), c)
	})
}
...
func TestComputePreviousCheckpointPath_Recovery(t *testing.T) {
	...
	reader := &nodeMocks.NodeStateReader{}
	reader.OnGetTaskNodeState().Return(handler.TaskNodeState{
		PreviousNodeExecutionCheckpointURI: storage.DataReference("s3://sandbox/x/prevname-n1-0/_nebulacheckpoints"),
	})
	nCtx.OnNodeStateReader().Return(reader)
	...
	t.Run("recovery-attempt-0-nCtx", func(t *testing.T) {
		c, err := ComputePreviousCheckpointPath(context.TODO(), 100, nCtx, "n1", 0)
		assert.NoError(t, err)
		assert.Equal(t, storage.DataReference("s3://sandbox/x/prevname-n1-0/_nebulacheckpoints"), c)
	})
