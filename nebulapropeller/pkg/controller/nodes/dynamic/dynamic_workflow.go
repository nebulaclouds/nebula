...
import (
	...
	node_common "github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/nodes/common"
...
func setEphemeralNodeExecutionStatusAttributes(ctx context.Context, djSpec *core.DynamicJobSpec,
	...
	// We keep track of the original node ids because that's where nebulakit inputs are written to in the case of legacy
...
func (d dynamicNodeTaskNodeHandler) buildContextualDynamicWorkflow(ctx context.Context, nCtx interfaces.NodeExecutionContext) (dynamicWorkflowContext, error) {
	...
	if ok, err := f.CacheExists(ctx); err != nil {
		...
		if err != nil {
			logger.Warnf(ctx, "Failed to load cached nebula workflow, this will cause the dynamic workflow to be recompiled. Error: %v", err)
...
func (d dynamicNodeTaskNodeHandler) buildDynamicWorkflow(ctx context.Context, nCtx interfaces.NodeExecutionContext,
	djSpec *core.DynamicJobSpec, dynamicNodeStatus v1alpha1.ExecutableNodeStatus) (*core.CompiledWorkflowClosure, *v1alpha1.NebulaWorkflow, error) {
	...
	// 	     See https://github.com/nebulaclouds/nebula/issues/219 for more information.
	...
	dynamicWf, err := k8s.BuildNebulaWorkflow(closure, &core.LiteralMap{}, nil, "")
	if err != nil {
