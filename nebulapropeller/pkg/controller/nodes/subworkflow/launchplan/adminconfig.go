...
import (
	ctrlConfig "github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/config"
)
...
// AdminConfig provides a "admin-launcher" section in core Nebulapropeller configuration and can be used to configure
// the rate at which Nebulapropeller can query for status of workflows in nebulaadmin or create new executions
type AdminConfig struct {
	// TPS indicates the maximum transactions per second to nebula admin from this client.
	// If it's zero, the created client will use DefaultTPS: 5
	TPS int64 `json:"tps" pflag:",The maximum number of transactions per second to nebula admin from this client."`
