...
type InmemoryWorkflowStore struct {
	store map[string]map[string]*v1alpha1.NebulaWorkflow
}

func (i *InmemoryWorkflowStore) Create(ctx context.Context, w *v1alpha1.NebulaWorkflow) error {
	if w != nil {
		if w.Name != "" && w.Namespace != "" {
			if _, ok := i.store[w.Namespace]; !ok {
				i.store[w.Namespace] = map[string]*v1alpha1.NebulaWorkflow{}
			}
...
func (i *InmemoryWorkflowStore) Get(ctx context.Context, namespace, name string) (*v1alpha1.NebulaWorkflow, error) {
	if m, ok := i.store[namespace]; ok {
		if v, ok := m[name]; ok {
			return v, nil
		}
...
func (i *InmemoryWorkflowStore) UpdateStatus(ctx context.Context, w *v1alpha1.NebulaWorkflow, priorityClass PriorityClass) (
	...
	if w != nil {
		if w.Name != "" && w.Namespace != "" {
			...
			return nil, kubeerrors.NewNotFound(v1alpha1.Resource(v1alpha1.NebulaWorkflowKind), w.Name)
		}
	}
...
func (i *InmemoryWorkflowStore) Update(ctx context.Context, w *v1alpha1.NebulaWorkflow, priorityClass PriorityClass) (
	newWF *v1alpha1.NebulaWorkflow, err error) {
