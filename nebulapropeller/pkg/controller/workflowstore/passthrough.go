...
type passthroughWorkflowStore struct {
	wfLister    listers.NebulaWorkflowLister
	wfClientSet v1alpha12.NebulaworkflowV1alpha1Interface
	metrics     *workflowstoreMetrics
}

func (p *passthroughWorkflowStore) Get(ctx context.Context, namespace, name string) (*v1alpha1.NebulaWorkflow, error) {
	w, err := p.wfLister.NebulaWorkflows(namespace).Get(name)
	if err != nil {
		// The NebulaWorkflow resource may no longer exist, in which case we stop
		// processing.
...
func (p *passthroughWorkflowStore) UpdateStatus(ctx context.Context, workflow *v1alpha1.NebulaWorkflow, priorityClass PriorityClass) (
	newWF *v1alpha1.NebulaWorkflow, err error) {
	p.metrics.workflowUpdateCount.Inc()
	// Something has changed. Lets save
	logger.Debugf(ctx, "Observed NebulaWorkflow State change. [%v] -> [%v]", workflow.Status.Phase.String(), workflow.Status.Phase.String())
	t := p.metrics.workflowUpdateLatency.Start()
	newWF, err = p.wfClientSet.NebulaWorkflows(workflow.Namespace).Update(ctx, workflow, v1.UpdateOptions{})
	if err != nil {
		if kubeerrors.IsNotFound(err) {
			return nil, nil
		}
...
func (p *passthroughWorkflowStore) Update(ctx context.Context, workflow *v1alpha1.NebulaWorkflow, priorityClass PriorityClass) (
	newWF *v1alpha1.NebulaWorkflow, err error) {
	// If the workflow has any managed fields setting the array to one empty ManagedField clears them in the CRD.
	// NebulaWorkflow CRDs are only managed by a single NebulaPropeller instance and therefore the managed fields paradigm
	...
	p.metrics.workflowUpdateCount.Inc()
	// Something has changed. Lets save
	logger.Debugf(ctx, "Observed NebulaWorkflow Update (maybe finalizer)")
	t := p.metrics.workflowUpdateLatency.Start()
	newWF, err = p.wfClientSet.NebulaWorkflows(workflow.Namespace).Update(ctx, workflow, v1.UpdateOptions{})
	if err != nil {
		if kubeerrors.IsNotFound(err) {
...
func NewPassthroughWorkflowStore(_ context.Context, scope promutils.Scope, wfClient v1alpha12.NebulaworkflowV1alpha1Interface,
	nebulaworkflowLister listers.NebulaWorkflowLister) NebulaWorkflow {
