...
// It is then entrypoint into the K8s based Nebula controller.
package controller
...
import (
	...
	clientset "github.com/nebulaclouds/nebula/nebulapropeller/pkg/client/clientset/versioned"
	informers "github.com/nebulaclouds/nebula/nebulapropeller/pkg/client/informers/externalversions"
	lister "github.com/nebulaclouds/nebula/nebulapropeller/pkg/client/listers/nebulaworkflow/v1alpha1"
	...
	"github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/nodes"
	"github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/nodes/catalog"
	errors3 "github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/nodes/errors"
	"github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/nodes/factory"
	...
	"github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/workflow"
	"github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/workflowstore"
	leader "github.com/nebulaclouds/nebula/nebulapropeller/pkg/leaderelection"
	"github.com/nebulaclouds/nebula/nebulapropeller/pkg/utils"
	"github.com/nebulaclouds/nebula/nebulastdlib/contextutils"
	stdErrs "github.com/nebulaclouds/nebula/nebulastdlib/errors"
	...
	k8sResolver "github.com/nebulaclouds/nebula/nebulastdlib/resolver"
...
// Controller is the controller implementation for NebulaWorkflow resources
type Controller struct {
	workerPool          *WorkerPool
	nebulaworkflowSynced cache.InformerSynced
	workQueue           CompositeWorkQueue
	gc                  *GarbageCollector
	numWorkers          int
	workflowStore       workflowstore.NebulaWorkflow
...
func (c *Controller) run(ctx context.Context) error {
	...
	logger.Info(ctx, "Starting NebulaWorkflow controller")
	return c.workerPool.Run(ctx, c.numWorkers, c.nebulaworkflowSynced)
}
...
// enqueueNebulaWorkflow takes a NebulaWorkflow resource and converts it into a namespace/name
// string which is then put onto the work queue. This method should *not* be
// passed resources of any type other than NebulaWorkflow.
func (c *Controller) enqueueNebulaWorkflow(obj interface{}) {
	ctx := context.TODO()
	wf, ok := obj.(*v1alpha1.NebulaWorkflow)
	if !ok {
...
// ResourceLevelMonitor is responsible for emitting metrics that show the current number of Nebula workflows,
...
type ResourceLevelMonitor struct {
	...
	// System Observability: This is a labeled gauge that emits the current number of NebulaWorkflow objects in the informer. It is used
	...
	lister lister.NebulaWorkflowLister
}

func (r *ResourceLevelMonitor) countList(ctx context.Context, workflows []*v1alpha1.NebulaWorkflow) map[string]map[string]int {
...
func NewResourceLevelMonitor(scope promutils.Scope, lister lister.NebulaWorkflowLister) *ResourceLevelMonitor {
	return &ResourceLevelMonitor{
		...
		levels:         labeled.NewGauge("nebulaworkflow", "Current NebulaWorkflow levels per instance of propeller", scope),
		lister:         lister,
	}
}
...
// New returns a new NebulaWorkflow controller
func New(ctx context.Context, cfg *config.Config, kubeClientset kubernetes.Interface, nebulapropellerClientset clientset.Interface,
	nebulaworkflowInformerFactory informers.SharedInformerFactory, informerFactory k8sInformers.SharedInformerFactory,
	...
	var launchPlanActor launchplan.NebulaAdmin
	...
	gc, err := NewGarbageCollector(cfg, scope, clock.RealClock{}, kubeClientset.CoreV1().Namespaces(), nebulapropellerClientset.NebulaworkflowV1alpha1())
	if err != nil {
	...
	// obtain references to shared index informers for NebulaWorkflow.
	nebulaworkflowInformer := nebulaworkflowInformerFactory.Nebulaworkflow().V1alpha1().NebulaWorkflows()
	controller.nebulaworkflowSynced = nebulaworkflowInformer.Informer().HasSynced
	...
	nebulak8s.DefaultPodTemplateStore.SetDefaultNamespace(podNamespace)
	...
	controller.workflowStore, err = workflowstore.NewWorkflowStore(ctx, workflowstore.GetConfig(), nebulaworkflowInformer.Lister(), nebulapropellerClientset.NebulaworkflowV1alpha1(), scope)
	if err != nil {
		return nil, stdErrs.Wrapf(errors3.CausedByError, err, "failed to initialize workflow store")
	}

	controller.levelMonitor = NewResourceLevelMonitor(scope.NewSubScope("collector"), nebulaworkflowInformer.Lister())
	...
	// Set up an event handler for when NebulaWorkflow resources change
	_, err = nebulaworkflowInformer.Informer().AddEventHandler(controller.getWorkflowUpdatesHandler())
	if err != nil {
		return nil, fmt.Errorf("failed to register event handler for NebulaWorkflow resource changes: %w", err)
	}

	updateHandler := nebulak8s.GetPodTemplateUpdatesHandler(&nebulak8s.DefaultPodTemplateStore)
...
// SharedInformerOptions creates informer options to work with NebulaPropeller Sharding
...
// StartController creates a new NebulaPropeller Controller and starts it
func StartController(ctx context.Context, cfg *config.Config, defaultNamespace string, mgr manager.Manager, scope *promutils.Scope) error {
	...
	nebulaworkflowClient, err := clientset.NewForConfig(kubecfg)
	if err != nil {
		return errors.Wrapf(err, "error building NebulaWorkflow clientset")
	}

	// Create NebulaWorkflow CRD if it does not exist
	if cfg.CreateNebulaWorkflowCRD {
		logger.Infof(ctx, "creating NebulaWorkflow CRD")
		...
		_, err = apiextensionsClient.ApiextensionsV1().CustomResourceDefinitions().Create(ctx, &nebulaworkflow.CRD, v1.CreateOptions{})
		if err != nil {
			if apierrors.IsAlreadyExists(err) {
				logger.Warnf(ctx, "NebulaWorkflow CRD already exists")
			} else {
				return errors.Wrapf(err, "failed to create NebulaWorkflow CRD")
			}
		}
	}

	opts := SharedInformerOptions(cfg, defaultNamespace)
	nebulaworkflowInformerFactory := informers.NewSharedInformerFactoryWithOptions(nebulaworkflowClient, cfg.WorkflowReEval.Duration, opts...)

	informerFactory := k8sInformers.NewSharedInformerFactoryWithOptions(kubeClient, nebulaK8sConfig.GetK8sPluginConfig().DefaultPodTemplateResync.Duration)

	c, err := New(ctx, cfg, kubeClient, nebulaworkflowClient, nebulaworkflowInformerFactory, informerFactory, mgr, *scope)
	if err != nil {
		return errors.Wrap(err, "failed to start NebulaPropeller")
	} else if c == nil {
		return errors.Errorf("Failed to create a new instance of NebulaPropeller")
	}

	go nebulaworkflowInformerFactory.Start(ctx.Done())
	go informerFactory.Start(ctx.Done())

	if err = c.Run(ctx); err != nil {
		return errors.Wrapf(err, "Error running NebulaPropeller.")
	}
	return nil
}
