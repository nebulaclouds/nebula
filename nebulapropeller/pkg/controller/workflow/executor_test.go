...
import (
	...
	eventMocks "github.com/nebulaclouds/nebula/nebulapropeller/events/mocks"
	...
	executorMocks "github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/executors/mocks"
	...
	gateMocks "github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/nodes/gate/mocks"
	"github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/nodes/handler"
	nodemocks "github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/nodes/interfaces/mocks"
	recoveryMocks "github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/nodes/recovery/mocks"
	"github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/nodes/subworkflow/launchplan"
	"github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/nodes/task/fakeplugins"
	wfErrors "github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/workflow/errors"
	...
	"github.com/nebulaclouds/nebula/nebulastdlib/yamlutils"
)
...
func TestWorkflowExecutor_HandleNebulaWorkflow_Error(t *testing.T) {
	...
	handlerFactory, err := factory.NewHandlerFactory(ctx, adminClient, adminClient, fakeKubeClient, mockClientset, catalogClient, recoveryClient, eventConfig, testClusterID, signalClient, promutils.NewTestScope())
	assert.NoError(t, err)
	...
	if assert.NoError(t, err) {
		...
		if assert.NoError(t, json.Unmarshal(wJSON, w)) {
			...
			for i := 0; i < 11; i++ {
				err := executor.HandleNebulaWorkflow(ctx, w)
...
func TestWorkflowExecutor_HandleNebulaWorkflow(t *testing.T) {
	ctx := context.Background()
	...
	b.Run("Test", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			for i := 0; i < 28; i++ {
				...
				err = executor.HandleNebulaWorkflow(ctx, deepW)
				if !assert.NoError(b, err) {
...
func TestWorkflowExecutor_HandleNebulaWorkflow_Failing(t *testing.T) {
	ctx := context.Background()
...
func TestWorkflowExecutor_HandleNebulaWorkflow_Events(t *testing.T) {
	ctx := context.Background()
	scope := promutils.NewTestScope()
	store := createInmemoryDataStore(t, scope)
	recorder := StdOutEventRecorder()
...
func TestWorkflowExecutor_HandleNebulaWorkflow_EventFailure(t *testing.T) {
	ctx := context.Background()
	scope := promutils.NewTestScope()
	store := createInmemoryDataStore(t, scope)
	recorder := StdOutEventRecorder()
