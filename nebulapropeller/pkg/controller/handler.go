...
import (
	...
	eventsErr "github.com/nebulaclouds/nebula/nebulapropeller/events/errors"
...
func RecordSystemError(w *v1alpha1.NebulaWorkflow, err error) *v1alpha1.NebulaWorkflow {
...
// Core Propeller structure that houses the Reconciliation loop for Nebulapropeller
type Propeller struct {
	store            *storage.DataStore
	wfStore          workflowstore.NebulaWorkflow
...
func SetDefinitionVersionIfEmpty(wf *v1alpha1.NebulaWorkflow, version v1alpha1.WorkflowDefinitionVersion) {
...
func (p *Propeller) TryMutateWorkflow(ctx context.Context, originalW *v1alpha1.NebulaWorkflow) (*v1alpha1.NebulaWorkflow, error) {
	...
	if !mutableW.GetExecutionStatus().IsTerminated() {
		...
		func() {
			...
			err = p.workflowExecutor.HandleNebulaWorkflow(ctx, mutableW)
		}()
		if err != nil {
...
// converge the two. It then updates the GetExecutionStatus block of the NebulaWorkflow resource
// with the current status of the resource.
// Every NebulaWorkflow transitions through the following
//
...
func (p *Propeller) Handle(ctx context.Context, namespace, name string) error {
	var span trace.Span
	ctx, span = otelutils.NewSpan(ctx, otelutils.NebulaPropellerTracer, "pkg.controller.Propeller/Handle")
	defer span.End()

	logger.Infof(ctx, "Processing Workflow.")
	defer logger.Infof(ctx, "Completed processing workflow.")

	// Get the NebulaWorkflow resource with this namespace/name
	_, wfStoreGetSpan := otelutils.NewSpan(ctx, otelutils.NebulaPropellerTracer, "WorkflowStore.Get")
	...
	// if the NebulaWorkflow CRD has the WorkflowClosureReference set then we have offloaded the
...
func (p *Propeller) parseWorkflowClosureCrdFields(ctx context.Context, dataReference storage.DataReference) (*k8s.WfClosureCrdFields, error) {
	_, span := otelutils.NewSpan(ctx, otelutils.NebulaPropellerTracer, "pkg.controller.Propeller/parseWorkflowClosureCrdFields")
	defer span.End()
...
func (p *Propeller) streak(ctx context.Context, w *v1alpha1.NebulaWorkflow, wfClosureCrdFields *k8s.WfClosureCrdFields) (*v1alpha1.NebulaWorkflow, error) {
	ctx, span := otelutils.NewSpan(ctx, otelutils.NebulaPropellerTracer, "pkg.controller.Propeller/streak")
	defer span.End()
	...
	// ExecutionNotFound error is returned when nebulaadmin is missing the workflow. This is not
	...
	if err != nil && eventsErr.IsNotFound(err) && w.GetExecutionStatus().GetPhase() != v1alpha1.WorkflowPhaseReady {
		...
		mutableW := w.DeepCopy()
		mutableW.Status.UpdatePhase(v1alpha1.WorkflowPhaseFailing, "Workflow execution is missing in nebulaadmin, aborting", &core.ExecutionError{
			Kind:    core.ExecutionError_SYSTEM,
			Code:    "ExecutionNotFound",
			Message: "Workflow execution not found in nebulaadmin.",
		})
	...
	// update the GetExecutionStatus block of the NebulaWorkflow resource. UpdateStatus will not
	// allow changes to the Spec of the resource, which is ideal for ensuring
	// nothing other than resource status has been updated.
	_, wfStoreUpdateSpan := otelutils.NewSpan(ctx, otelutils.NebulaPropellerTracer, "WorkflowStore.Update")
	...
	if updateErr != nil {
		...
		if workflowstore.IsWorkflowTooLarge(updateErr) {
			...
			mutableW.Status.UpdatePhase(v1alpha1.WorkflowPhaseFailing, "Workflow size has breached threshold, aborting", &core.ExecutionError{
				...
				Message: "Workflow execution state is too large for Nebula to handle.",
			})
			if _, e := p.wfStore.Update(ctx, mutableW, workflowstore.PriorityClassCritical); e != nil {
...
func NewPropellerHandler(_ context.Context, cfg *config.Config, store *storage.DataStore, wfStore workflowstore.NebulaWorkflow, executor executors.Workflow, scope promutils.Scope) *Propeller {
