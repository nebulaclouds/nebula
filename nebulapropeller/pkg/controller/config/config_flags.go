...
func (cfg Config) GetPFlagSet(prefix string) *pflag.FlagSet {
	...
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "metadata-prefix"), defaultConfig.MetadataPrefix, "MetadataPrefix should be used if all the metadata for Nebula executions should be stored under a specific prefix in CloudStorage. If not specified,  the data will be stored in the base container directly.")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "rawoutput-prefix"), defaultConfig.DefaultRawOutputPrefix, "a fully qualified storage path of the form s3://nebula/abc/...,  where all data sandboxes should be stored.")
	...
	cmdFlags.StringSlice(fmt.Sprintf("%v%v", prefix, "include-shard-key-label"), defaultConfig.IncludeShardKeyLabel, "Include the specified shard key label in the k8s NebulaWorkflow CRD label selector")
	cmdFlags.StringSlice(fmt.Sprintf("%v%v", prefix, "exclude-shard-key-label"), defaultConfig.ExcludeShardKeyLabel, "Exclude the specified shard key label from the k8s NebulaWorkflow CRD label selector")
	cmdFlags.StringSlice(fmt.Sprintf("%v%v", prefix, "include-project-label"), defaultConfig.IncludeProjectLabel, "Include the specified project label in the k8s NebulaWorkflow CRD label selector")
	cmdFlags.StringSlice(fmt.Sprintf("%v%v", prefix, "exclude-project-label"), defaultConfig.ExcludeProjectLabel, "Exclude the specified project label from the k8s NebulaWorkflow CRD label selector")
	cmdFlags.StringSlice(fmt.Sprintf("%v%v", prefix, "include-domain-label"), defaultConfig.IncludeDomainLabel, "Include the specified domain label in the k8s NebulaWorkflow CRD label selector")
	cmdFlags.StringSlice(fmt.Sprintf("%v%v", prefix, "exclude-domain-label"), defaultConfig.ExcludeDomainLabel, "Exclude the specified domain label from the k8s NebulaWorkflow CRD label selector")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "cluster-id"), defaultConfig.ClusterID, "Unique cluster id running this nebulapropeller instance with which to annotate execution events")
	cmdFlags.Bool(fmt.Sprintf("%v%v", prefix, "create-nebulaworkflow-crd"), defaultConfig.CreateNebulaWorkflowCRD, "Enable creation of the NebulaWorkflow CRD on startup")
