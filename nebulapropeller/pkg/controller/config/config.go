// Package config contains the core configuration for NebulaPropeller. This configuration can be added under the “propeller“ section.
//
//	Example config:
//
// ----------------
...
//	       rate: 1000
//	       capacity: 10000
//	   # This config assumes using `make start` in nebulasnacks repo to startup a DinD k3s container
...
var (
	...
	defaultConfig = &Config{
		...
		ClusterID:              "propeller",
		CreateNebulaWorkflowCRD: false,
		ArrayNodeEventVersion:  0,
	}
)

// Config that uses the nebulastdlib Config module to generate commandline and load config files. This configuration is
...
type Config struct {
	...
	MetadataPrefix           string               `json:"metadata-prefix,omitempty" pflag:",MetadataPrefix should be used if all the metadata for Nebula executions should be stored under a specific prefix in CloudStorage. If not specified, the data will be stored in the base container directly."`
	DefaultRawOutputPrefix   string               `json:"rawoutput-prefix" pflag:",a fully qualified storage path of the form s3://nebula/abc/..., where all data sandboxes should be stored."`
	...
	IncludeShardKeyLabel     []string             `json:"include-shard-key-label" pflag:",Include the specified shard key label in the k8s NebulaWorkflow CRD label selector"`
	ExcludeShardKeyLabel     []string             `json:"exclude-shard-key-label" pflag:",Exclude the specified shard key label from the k8s NebulaWorkflow CRD label selector"`
	IncludeProjectLabel      []string             `json:"include-project-label" pflag:",Include the specified project label in the k8s NebulaWorkflow CRD label selector"`
	ExcludeProjectLabel      []string             `json:"exclude-project-label" pflag:",Exclude the specified project label from the k8s NebulaWorkflow CRD label selector"`
	IncludeDomainLabel       []string             `json:"include-domain-label" pflag:",Include the specified domain label in the k8s NebulaWorkflow CRD label selector"`
	ExcludeDomainLabel       []string             `json:"exclude-domain-label" pflag:",Exclude the specified domain label from the k8s NebulaWorkflow CRD label selector"`
	ClusterID                string               `json:"cluster-id" pflag:",Unique cluster id running this nebulapropeller instance with which to annotate execution events"`
	CreateNebulaWorkflowCRD   bool                 `json:"create-nebulaworkflow-crd" pflag:",Enable creation of the NebulaWorkflow CRD on startup"`
...
// KubeClientConfig contains the configuration used by nebulapropeller to configure its internal Kubernetes Client.
...
// GetConfig extracts the Configuration from the global config module in nebulastdlib and returns the corresponding type-casted object.
