...
import (
	...
	"github.com/nebulaclouds/nebula/nebulapropeller/pkg/compiler/typing"
)

func validateBinding(w c.WorkflowBuilder, nodeID c.NodeID, nodeParam string, binding *nebula.BindingData,
	expectedType *nebula.LiteralType, errs errors.CompileErrors, validateParamTypes bool) (
	...
	default:
		if expectedType.GetUnionType() != nil {
			var matchingType *nebula.LiteralType
			var resolvedType *nebula.LiteralType
	...
	case *nebula.BindingData_Collection:
		...
		if expectedType.GetCollectionType() != nil {
			...
			var subType *nebula.LiteralType
			...
			return &nebula.LiteralType{
				Type: &nebula.LiteralType_CollectionType{
	...
	case *nebula.BindingData_Map:
		...
		if expectedType.GetMapValueType() != nil {
			...
			return &nebula.LiteralType{
				Type: &nebula.LiteralType_MapValueType{
		...
		if upNode, found := validateNodeID(w, val.Promise.NodeId, errs.NewScope()); found {
			...
			if param, paramFound := validateOutputVar(upNode, v.Name, errs.NewScope()); paramFound {
				...
				for _, attr := range val.Promise.AttrPath {
					var tmpType *nebula.LiteralType
					var exist bool
...
func ValidateBindings(w c.WorkflowBuilder, node c.Node, bindings []*nebula.Binding, params *nebula.VariableMap,
	validateParamTypes bool, edgeDirection c.EdgeDirection, errs errors.CompileErrors) (resolved *nebula.VariableMap, ok bool) {

	resolved = &nebula.VariableMap{
		Variables: make(map[string]*nebula.Variable, len(bindings)),
	}
	...
	for _, binding := range bindings {
		if param, ok := findVariableByName(params, binding.GetVar()); !ok && validateParamTypes {
			...
			if !validateParamTypes && param == nil {
				param = &nebula.Variable{
					Type: &nebula.LiteralType{
						Type: &nebula.LiteralType_Simple{},
					},
				}
			}
			...
			if resolvedType, upstreamNodes, bindingOk := validateBinding(w, node.GetId(), binding.GetVar(), binding.GetBinding(),
				...
				resolved.Variables[binding.GetVar()] = &nebula.Variable{
					Type: resolvedType,
				}
			}
		}
...
func IsOptionalType(variable nebula.Variable) bool {
	if variable.Type.GetUnionType() == nil {
		return false
	}
	for _, variant := range variable.Type.GetUnionType().Variants {
		if nebula.SimpleType_NONE == variant.GetSimple() {
			return true
		}
	}
	return false
