...
func validateBranchInterface(w c.WorkflowBuilder, node c.NodeBuilder, errs errors.CompileErrors) (iface *nebula.TypedInterface, ok bool) {
	...
	var outputs map[string]*nebula.Variable
	finalOutputParameterNames := sets.NewString()

	validateIfaceMatch := func(nodeId string, iface2 *nebula.TypedInterface, errsScope errors.CompileErrors) (match bool) {
	...
	cases := make([]*nebula.Node, 0, len(node.GetBranchNode().IfElse.Other)+1)
	...
	for _, block := range cases {
		...
		iface2 = &nebula.TypedInterface{
			Inputs:  &nebula.VariableMap{Variables: map[string]*nebula.Variable{}},
	...
	inputVarsFromBindings, _ := ValidateBindings(w, node, node.GetInputs(), &nebula.VariableMap{Variables: map[string]*nebula.Variable{}},
		false, c.EdgeDirectionUpstream, errs.NewScope())

	if !errs.HasErrors() && iface != nil {
		iface = &nebula.TypedInterface{
