...
const (
	// Labels are set on the NebulaWorkflow CRD to aid downstream processing

	// The NebulaWorkflow domain according to registration ownership
	DomainLabel = "domain"
	// A concatenation of project, domain, workflow name, and a unique ID
	ExecutionIDLabel = "execution-id"
	// The NebulaWorkflow project according to registration ownership
	ProjectLabel = "project"
	// Shard keys are used during NebulaPropeller sharding, this value is set to a hash of the NebulaWorkflow ExecutionID.
	...
	// The fully qualified NebulaWorkflow name
...
func buildNebulaWorkflowSpec(wf *core.CompiledWorkflow, tasks []*core.CompiledTask, errs errors.CompileErrors) (
...
// BuildNebulaWorkflow builds v1alpha1.NebulaWorkflow resource. Returned error, if not nil, is of type errors.CompilerErrors.
func BuildNebulaWorkflow(wfClosure *core.CompiledWorkflowClosure, inputs *core.LiteralMap,
	executionID *core.WorkflowExecutionIdentifier, namespace string) (*v1alpha1.NebulaWorkflow, error) {
	...
	primarySpec, err := buildNebulaWorkflowSpec(wfClosure.Primary, tasks, errs.NewScope())
	if err != nil {
	...
	for _, subWf := range wfClosure.SubWorkflows {
		spec, err := buildNebulaWorkflowSpec(subWf, tasks, errs.NewScope())
		if err != nil {
			errs.Collect(errors.NewWorkflowBuildError(err))
		} else {
	...
	obj := &v1alpha1.NebulaWorkflow{
		TypeMeta: v1.TypeMeta{
			Kind:       v1alpha1.NebulaWorkflowKind,
...
func BuildWfClosureCrdFields(wfClosure *core.CompiledWorkflowClosure) (*WfClosureCrdFields, error) {
	...
	primarySpec, err := buildNebulaWorkflowSpec(wfClosure.Primary, wfClosure.Tasks, errs.NewScope())
	if err != nil {
		errs.Collect(errors.NewWorkflowBuildError(err))
		return nil, errs
	}
	...
	subwfs := make(map[v1alpha1.WorkflowID]*v1alpha1.WorkflowSpec, len(wfClosure.SubWorkflows))
	for _, subWf := range wfClosure.SubWorkflows {
		spec, err := buildNebulaWorkflowSpec(subWf, wfClosure.Tasks, errs.NewScope())
		if err != nil {
			errs.Collect(errors.NewWorkflowBuildError(err))
		} else {
			subwfs[subWf.Template.Id.String()] = spec
