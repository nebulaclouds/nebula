...
type nebulaTask = core.TaskTemplate //nolint:unused
type nebulaWorkflow = core.CompiledWorkflow
type nebulaNode = core.Node

// A builder object for the Graph struct. This contains information the compiler uses while building the final Graph
// struct.
type workflowBuilder struct {
	CoreWorkflow     *nebulaWorkflow
...
func (w workflowBuilder) GetOrCreateNodeBuilder(n *nebulaNode) c.NodeBuilder {
	address := fmt.Sprintf("%p", n)
	if existingBuilder, found := w.NodeBuilderIndex[address]; found {
		return existingBuilder
	}

	newObj := &nodeBuilder{nebulaNode: n}
...
func (w workflowBuilder) GetCoreWorkflow() *nebulaWorkflow {
...
func (t taskBuilder) GetID() c.Identifier {
	if t.nebulaTask.Id != nil {
		return *t.nebulaTask.Id
	}
