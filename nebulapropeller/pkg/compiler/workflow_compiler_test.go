...
import (
	...
	"github.com/nebulaclouds/nebula/nebulapropeller/pkg/compiler/common/mocks"
...
func TestComparisonExpression_MissingLeftRight(t *testing.T) {
	...
	v.ValidateBooleanExpression(w, &nodeBuilder{nebulaNode: &nebulaNode{}}, bExpr, true, errs)
	assert.Error(t, errs)
...
func TestValidateUnderlyingInterface(parentT *testing.T) {
	...
	parentT.Run("TaskNode", func(t *testing.T) {
		errs := errors.NewCompileErrors()
		iface, ifaceOk := v.ValidateUnderlyingInterface(&g, &nodeBuilder{nebulaNode: inputWorkflow.Nodes[0]}, errs)
	...
	parentT.Run("GraphNode", func(t *testing.T) {
		errs := errors.NewCompileErrors()
		iface, ifaceOk := v.ValidateUnderlyingInterface(&g, &nodeBuilder{nebulaNode: &core.Node{
	...
	parentT.Run("BranchNode", func(branchT *testing.T) {
		...
		branchT.Run("TwoCases", func(t *testing.T) {
			errs := errors.NewCompileErrors()
			_, ifaceOk := v.ValidateUnderlyingInterface(&g, &nodeBuilder{nebulaNode: &core.Node{
				Target: &core.Node_BranchNode{
					BranchNode: &core.BranchNode{
						IfElse: &core.IfElseBlock{
							Case: &core.IfBlock{
