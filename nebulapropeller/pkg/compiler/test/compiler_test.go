...
func TestDynamic(t *testing.T) {
	...
	assert.NoError(t, filepath.Walk("testdata/dynamic", func(path string, info os.FileInfo, err error) error {
		...
		t.Run(path, func(t *testing.T) {
			...
			nebulaWf, err := k8s.BuildNebulaWorkflow(compiledWfc,
				inputs,
	...
	t.Run("k8s-"+dirName, func(t *testing.T) {
		...
		for _, p := range paths {
			t.Run(p, func(t *testing.T) {
				...
				if !storeOrDiff(t, yaml.Marshal, nebulaWf, filepath.Join(filepath.Dir(filepath.Dir(p)), "k8s", strings.TrimRight(filepath.Base(p), filepath.Ext(p))+".yaml")) {
					t.FailNow()
				}
			})
		}
