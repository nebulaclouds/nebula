// Package compiler provides compiler services for nebula workflows. It performs static analysis on the Workflow and produces
// CompilerErrors for any detected issue. A nebula workflow should only be considered valid for execution if it passed through
...
import (
	...
	v "github.com/nebulaclouds/nebula/nebulapropeller/pkg/compiler/validators"
)
...
func (w workflowBuilder) ValidateWorkflow(fg *nebulaWorkflow, errs errors.CompileErrors) (c.Workflow, bool) {
...
// CompileWorkflow compiles a nebula workflow a and all of its dependencies into a single executable Workflow. Refer to
...
func CompileWorkflow(primaryWf *core.WorkflowTemplate, subworkflows []*core.WorkflowTemplate, tasks []*core.CompiledTask,
	...
	for _, task := range tasks {
		...
		taskBuilders = append(taskBuilders, &taskBuilder{nebulaTask: task.Template})
...
func (w workflowBuilder) newWorkflowBuilder(fg *nebulaWorkflow) workflowBuilder {
	return newWorkflowBuilder(fg, w.allSubWorkflows, w.allTasks, w.allLaunchPlans)
}

func newWorkflowBuilder(fg *nebulaWorkflow, wfIndex c.WorkflowIndex, tasks c.TaskIndex,
