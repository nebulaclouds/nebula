...
import (
	...
	nebulaScheme "github.com/nebulaclouds/nebula/nebulapropeller/pkg/client/clientset/versioned/scheme"
	"github.com/nebulaclouds/nebula/nebulapropeller/pkg/controller/config"
	"github.com/nebulaclouds/nebula/nebulastdlib/logger"
)

var NotTheOwnerError = errors.Errorf("NebulaPropeller is not the owner")
...
func GetWorkflowIDFromOwner(reference *metav1.OwnerReference, namespace string) (v1alpha1.WorkflowID, error) {
	...
	if reference.Kind == v1alpha1.NebulaWorkflowKind {
...
func NewK8sEventRecorder(ctx context.Context, kubeclientset kubernetes.Interface, controllerAgentName string, publishK8sEvents bool) (record.EventRecorder, error) {
	...
	// Add NebulaWorkflow controller types to the default Kubernetes Scheme so Events can be
	// logged for NebulaWorkflow Controller types.
	err := nebulaScheme.AddToScheme(scheme.Scheme)
	if err != nil {
		return nil, err
	}
