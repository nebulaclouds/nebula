	...
	expected := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Annotations: map[string]string{
				...
				"vault.hashicorp.com/secret-volume-path":                          "/etc/nebula/secrets",
				"vault.hashicorp.com/role":                                        "nebula",
...
func TestVaultSecretManagerInjector_Inject(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				cfg: config.VaultSecretManagerConfig{Role: "nebula", KVVersion: config.KVVersion2},
				secret: &coreIdl.Secret{
					Group:        "foo",
					Key:          "bar",
					GroupVersion: "kv1",
		...
		},
		{
			...
			args: args{
				cfg: config.VaultSecretManagerConfig{Role: "nebula", KVVersion: config.KVVersion1},
				secret: &coreIdl.Secret{
					Group:        "foo",
					Key:          "bar",
					GroupVersion: "kv2",
		...
		},
		{
			...
			args: args{
				cfg: config.VaultSecretManagerConfig{Role: "nebula", KVVersion: config.KVVersion2, Annotations: map[string]string{
					"vault.hashicorp.com/auth-config-type": "gce",
				}},
				secret: inputSecret,
				p:      NewInputPod(map[string]string{}),
