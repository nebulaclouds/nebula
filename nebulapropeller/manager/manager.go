...
import (
	...
	"github.com/nebulaclouds/nebula/nebulapropeller/manager/shardstrategy"
	...
	stderrors "github.com/nebulaclouds/nebula/nebulastdlib/errors"
...
// Manager periodically scans k8s to ensure liveness of multiple NebulaPropeller controller instances
...
func (m *Manager) createPods(ctx context.Context) error {
	...
	// unmanaged nebulapropeller pods - which is invalid configuration but will be detected later
	...
	for i, podName := range podNames {
		if exists := podExists[podName]; !exists {
			...
			// override leader election namespaced name on managed nebulapropeller instances
			container, err := utils.GetContainer(&pod.Spec, m.podTemplateContainerName)
			if err != nil {
				return fmt.Errorf("failed to retrieve nebulapropeller container from pod template [%v]", err)
			}
...
// New creates a new NebulaPropeller Manager instance.
func New(ctx context.Context, propellerCfg *propellerConfig.Config, cfg *managerConfig.Config, podNamespace string, ownerReferences []metav1.OwnerReference, kubeClient kubernetes.Interface, scope promutils.Scope) (*Manager, error) {
	...
	eventRecorder, err := utils.NewK8sEventRecorder(ctx, kubeClient, "nebulapropeller-manager", propellerCfg.PublishK8sEvents)
	if err != nil {
