...
import (
	...
	"github.com/nebulaclouds/nebula/nebulapropeller/manager/shardstrategy/mocks"
		...
		Template: v1.PodTemplateSpec{
			...
			Spec: v1.PodSpec{
				Containers: []v1.Container{
					v1.Container{
						Command: []string{"nebulapropeller"},
						Args:    []string{"--config", "/etc/nebula/config/*.yaml"},
					},
				},
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, pod := range pods.Items {
				...
				assert.Equal(t, pod.Labels["app"], "nebulapropeller")
			...
			for i := 0; i < tt.shardStrategy.GetPodCount(); i++ {
				initObjects = append(initObjects, &v1.Pod{
					ObjectMeta: metav1.ObjectMeta{
						...
						Name: fmt.Sprintf("nebulapropeller-%d", i),
					},
				})
			}
