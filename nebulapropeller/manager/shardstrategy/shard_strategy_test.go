...
import (
	...
	"github.com/nebulaclouds/nebula/nebulapropeller/manager/config"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			for podIndex := 0; podIndex < tt.shardStrategy.GetPodCount(); podIndex++ {
				...
				err := tt.shardStrategy.UpdatePodSpec(&podSpec, "nebulapropeller", podIndex)
...
func TestUpdatePodSpecInvalidPodIndex(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			lowerErr := tt.shardStrategy.UpdatePodSpec(&podSpec, "nebulapropeller", -1)
			assert.Error(t, lowerErr)

			upperErr := tt.shardStrategy.UpdatePodSpec(&podSpec, "nebulapropeller", tt.shardStrategy.GetPodCount())
...
func TestUpdatePodSpecInvalidPodSpec(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := tt.shardStrategy.UpdatePodSpec(&podSpec, "nebulapropeller", 0)
			assert.Error(t, err)
		})
	}
}
...
var (
	...
	projectShardConfig = config.ShardConfig{
		...
		PerShardMappings: []config.PerShardMappingsConfig{
			{IDs: []string{"nebulasnacks"}},
			{IDs: []string{"nebulafoo", "nebulabar"}},
		},
	}
