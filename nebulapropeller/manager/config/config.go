...
var (
	DefaultConfig = &Config{
		PodApplication:           "nebulapropeller",
		PodTemplateContainerName: "nebulapropeller",
		PodTemplateName:          "nebulapropeller-template",
		PodTemplateNamespace:     "nebula",
...
// Configuration for the NebulaPropeller sharding strategy
...
// Configuration for the NebulaPropeller Manager instance
type Config struct {
	PodApplication           string          `json:"pod-application" pflag:",Application name for managed pods"`
	PodTemplateContainerName string          `json:"pod-template-container-name" pflag:",The container name within the K8s PodTemplate name used to set NebulaWorkflow CRD labels selectors"`
	PodTemplateName          string          `json:"pod-template-name" pflag:",K8s PodTemplate name to use for starting NebulaPropeller pods"`
	PodTemplateNamespace     string          `json:"pod-template-namespace" pflag:",Namespace where the k8s PodTemplate is located"`
	ScanInterval             config.Duration `json:"scan-interval" pflag:",Frequency to scan NebulaPropeller pods and start / restart if necessary"`
