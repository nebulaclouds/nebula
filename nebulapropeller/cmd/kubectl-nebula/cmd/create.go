...
import (
	...
	compilerErrors "github.com/nebulaclouds/nebula/nebulapropeller/pkg/compiler/errors"
...
func (c *CreateOpts) createWorkflowFromProto() error {
	...
	nebulaWf, err := k8s.BuildNebulaWorkflow(wf, inputs, executionID, c.ConfigOverrides.Context.Namespace)
	if err != nil {
		return err
	}
	nebulaWf.ExecutionID = v1alpha1.WorkflowExecutionIdentifier{
		WorkflowExecutionIdentifier: executionID,
	}
	if nebulaWf.Annotations == nil {
		nebulaWf.Annotations = *c.annotations.value
	} else {
		for key, val := range *c.annotations.value {
			nebulaWf.Annotations[key] = val
		}
	}

	if c.dryRun {
		fmt.Printf("Dry Run mode enabled. Printing the compiled workflow.\n")
		j, err := json.Marshal(nebulaWf)
		if err != nil {
		...
		wf, err := c.nebulaClient.NebulaworkflowV1alpha1().NebulaWorkflows(c.ConfigOverrides.Context.Namespace).Create(context.TODO(), nebulaWf, v1.CreateOptions{})
		if err != nil {
			return err
		}

		fmt.Printf("Successfully created Nebula Workflow %v.\n", wf.Name)
	}
	return nil
}
