...
import (
	...
	"github.com/nebulaclouds/nebula/nebulapropeller/cmd/kubectl-nebula/cmd/printers"
...
func (g *GetOpts) getWorkflow(ctx context.Context, name string) error {
	...
	w, err := g.nebulaClient.NebulaworkflowV1alpha1().NebulaWorkflows(g.ConfigOverrides.Context.Namespace).Get(context.TODO(), name, v1.GetOptions{})
	if err != nil {
		return err
	}
...
func (g *GetOpts) iterateOverWorkflows(f func(*v1alpha1.NebulaWorkflow) error, batchSize int64, limit int64) error {
	...
	var counter int64
	for {
		wList, err := g.nebulaClient.NebulaworkflowV1alpha1().NebulaWorkflows(g.ConfigOverrides.Context.Namespace).List(context.TODO(), *opts)
		if err != nil {
			return err
		}
...
func (g *GetOpts) listWorkflows() error {
	...
	err := g.iterateOverWorkflows(
		func(w *v1alpha1.NebulaWorkflow) error {
			counter++
